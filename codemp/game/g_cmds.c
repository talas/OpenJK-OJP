	...
	*/
	...
	if ( give_all || !Q_stricmp( name, "ammo" ) )
	{
		...
		for ( i=AMMO_BLASTER; i<AMMO_MAX ; i++ )
			Add_Ammo( ent, i, Com_Clampi( 0, ammoData[i].max, num ) );
		if ( !give_all )
			return;
	}
	//[/CoOp]
...
argCheck:
	...
	*/
	...
	if (trap->Argc() > 1)
	{
		...
		trap->Argv( 1, arg, sizeof( arg ) );
		if (arg[0] && arg[0] != 'x' && level.gametype != GT_DUEL && level.gametype != GT_POWERDUEL)
...
void G_Say( gentity_t *ent, gentity_t *target, int mode, const char *chatText ) {
	...
	//[TABBot]
	//Scan for bot orders
	BotOrderParser(ent, target, mode, text);
	//[/TABBot]
...
void Cmd_ModelScale_f( gentity_t *ent ) {
	int size;
//	int temp,temp2;
	char	cmd2[MAX_TOKEN_CHARS] = {0};
	if ( trap->Argc() > 1 )
	{
		if(!ojp_modelscaleEnabled.integer)
		{
			trap->SendServerCommand( ent-g_entities, va("print \"Modelscale is disabled!\n\"") );
			return;
		}
		if(trap->Argc()!=2)
		{
			trap->SendServerCommand( ent-g_entities, va("print \"Current modelscale is %i.\n\"", (ent->client->ps.iModelScale ? ent->client->ps.iModelScale : 100)) );
			return;
		}
		trap->Argv(1, cmd2, sizeof(cmd2));
		size=atoi(cmd2);
...
void Cmd_TestTrace_f( gentity_t *ent ) {
	//[Test]
#ifndef FINAL_BUILD
	trace_t tr;
	...
	trap->Trace( &tr, traceFrom, NULL, NULL, traceTo, ent->s.number, MASK_SHOT, qfalse, 0, 0 );

	if(tr.fraction < 1.0f)
	{
		Com_Printf("%i", tr.entityNum);
	}
#endif
	//[/Test]
}
...
void Cmd_LamerCheck_f( gentity_t *ent ) {
	trap->SendServerCommand( -1, va("cp \"This game is based on code taken from the\nOpen Jedi Project and OpenJK. Please give proper credit.\n\""));
}

//[HolocronFiles]
void Cmd_AddHolocron_f( gentity_t *ent ) {
	// Add a new holocron point. Unique1 added.
	AOTCTC_Holocron_Add ( ent );
}

void Cmd_SaveHolocrons_f( gentity_t *ent ) {
	// Save holocron position table. Unique1 added.
	AOTCTC_Holocron_Savepositions();		
}

void Cmd_SpawnHolocron_f( gentity_t *ent ) {
...
//[CoOpEditor]
void Cmd_AutoSaveAdd_f( gentity_t *ent ) {
	int args = trap->Argc();
...
void Cmd_AutoSaveSave_f( gentity_t *ent ) {
	Save_Autosaves();
}

void Cmd_AutoSaveDelete_f( gentity_t *ent ) {
...
command_t commands[] = {
	...
	{ "addholocron",			Cmd_AddHolocron_f,		CMD_CHEAT|CMD_NOINTERMISSION },
	{ "autosave_add",			Cmd_AutoSaveAdd_f,		CMD_CHEAT|CMD_NOINTERMISSION },
	{ "autosave_save",			Cmd_AutoSaveSave_f,		CMD_CHEAT|CMD_NOINTERMISSION },
	{ "autosave_delete",			Cmd_AutoSaveDelete_f,		CMD_CHEAT|CMD_NOINTERMISSION },
...
//	{ "debugSetSaberBlock",	Cmd_DebugSetSaberBlock_f,		CMD_CHEAT|CMD_ALIVE },
...
//	{ "kylesmash",			TryGrapple,					0 },
	...
	{ "modelscale",			Cmd_ModelScale_f,				0 },
	{ "noclip",				Cmd_Noclip_f,				CMD_CHEAT|CMD_ALIVE|CMD_NOINTERMISSION },
	{ "notarget",			Cmd_Notarget_f,				CMD_CHEAT|CMD_ALIVE|CMD_NOINTERMISSION },
	{ "npc",				Cmd_NPC_f,					CMD_CHEAT|CMD_ALIVE },
	{ "saveholocrons",			Cmd_SaveHolocrons_f,			CMD_CHEAT|CMD_NOINTERMISSION },
	{ "say",				Cmd_Say_f,					0 },
	{ "say_team",			Cmd_SayTeam_f,				0 },
	{ "spawnholocron",			Cmd_SpawnHolocron_f,			CMD_CHEAT|CMD_NOINTERMISSION },
	{ "score",				Cmd_Score_f,				0 },
...
//	{ "teamtask",			Cmd_TeamTask_f,				CMD_NOINTERMISSION },
	...
	{ "tell",				Cmd_Tell_f,					0 },
	{ "testtrace",			Cmd_TestTrace_f,				CMD_CHEAT|CMD_NOINTERMISSION },
	{ "thedestroyer",		Cmd_TheDestroyer_f,			CMD_CHEAT|CMD_ALIVE|CMD_NOINTERMISSION },
	{ "togglesaber",		Cmd_ToggleSaber_f,			CMD_ALIVE|CMD_NOINTERMISSION },
