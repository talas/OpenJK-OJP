...
{
	//[RAFIXME] impliment this stuff somehow?
	/*
...
{//remember this alert.  Also add this to NPC's danger sense for navigation purposes.
	alertEvent_t *at = NULL;
	qboolean	IsDangerous;
	qboolean	IsFromNPC;
	qboolean	IsFromEnemy;
	...
	//----------------------
	at = &level.alertEvents[alertIndex];
...
{//special-case alerts that would be a pain in the ass to have the ent's think funcs generate
	...
	if ( ent->s.eType == ET_MISSILE && ent->s.weapon == WP_THERMAL && ent->s.pos.trType == TR_STATIONARY )
	{
		...
		{//events were just cleared out so add me again
			AddSoundEvent( &g_entities[ent->s.owner], ent->r.currentOrigin, ent->splashRadius*2, AEL_DANGER, qfalse, qtrue );
			AddSightEvent( &g_entities[ent->s.owner], ent->r.currentOrigin, ent->splashRadius*2, AEL_DANGER, 0 );
		}
	}
	/* RAFIXME - impliment this forcePushTime check?
	...
	{//being pushed
		...
		{//events were just cleared out so add me again
			...
			{
				AddSightEvent( player, ent->r.currentOrigin, 128, AEL_DISCOVERED );//security keys are more important
			}
			else
			{
				AddSightEvent( player, ent->r.currentOrigin, 128, AEL_SUSPICIOUS );//hmm... or should this always be discovered?
			}
		}
	}
	*/
