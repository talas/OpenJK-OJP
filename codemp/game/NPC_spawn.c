...
//			RegisterItem( FindItemForWeapon( (weapon_t)(curWeap) ) );	//precache the weapon
			...
			ent->NPC->currentAmmo = ent->client->ps.ammo[curWeap] = 100;//FIXME: max ammo
...
//	} else
	...
	*/
	...
	//select the weapon
	ent->NPC->currentAmmo = ent->client->ps.ammo[ent->client->ps.weapon];
...
qboolean NPC_SafeSpawn( gentity_t *ent, float safeRadius )
{
	...
	numEnts = trap->EntitiesInBox( safeMins, safeMaxs, radiusEnts, MAX_SAFESPAWN_ENTS );

	for ( i = 0; i < numEnts; i++ )
	{
		FoundEnt = &g_entities[radiusEnts[i]];
		...
		distance = DistanceSquared( ent->r.currentOrigin, FoundEnt->r.currentOrigin );
...
*/
	//[CoOp]
	//flag used to prevent NPCs from trying to spawn on top of each other.
...
void SP_NPC_Droid_Remote( gentity_t *self)
{
	//[CoOp]
	//RAFIXME
	//Instead of using a completely different NPC class
	//I've hacked this to use the actual remote AI with
	//a howler body in this NPC's file
	//for the sake of getting this to run.
	//I'll hopefully fix this later.
...
void Cmd_NPC_f( gentity_t *ent )
{
	...
	{
		Com_Printf( "ERROR: Cmd_NPC_f cannot be used during cutscenes\n" );
	}
