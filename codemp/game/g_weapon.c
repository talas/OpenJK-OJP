...
//---------------------------------------------------------
{
	...
	{//adding the primary fire inaccuracy.  It's slight enough that it won't affect at closer ranges, but it keeps folks on their toes, since they can't snipe from across the levels anymore.                                                       
		angs[PITCH] += Q_flrand(-1.0f, 1.0f) * BLASTER_SPREAD2;
		angs[YAW]	+= Q_flrand(-1.0f, 1.0f) * BLASTER_SPREAD2;
	}
	//[/WeaponSys]
...
//---------------------------------------------------------
{
	...
	{//need to loop this in case we hit a Jedi who dodges the shot
		...
		{
			//[WeaponSys]
			trap->Trace( &tr, start, shotMins, shotMaxs, end, ignore, MASK_SHOT, qfalse, G2TRFLAG_DOGHOULTRACE|G2TRFLAG_GETSURFINDEX|G2TRFLAG_THICK|G2TRFLAG_HITCORPSES, g_g2TraceLod.integer );
			//[/WeaponSys]
		}
		else
		{
			//[WeaponSys]
			trap->Trace( &tr, start, shotMins, shotMaxs, end, ignore, MASK_SHOT, qfalse, 0, 0 );
			//[/WeaponSys]
		}
		...
		{ //g2 collision checks -rww
			...
			{ //since we used G2TRFLAG_GETSURFINDEX, tr.surfaceFlags will actually contain the index of the surface on the ghoul2 model we collided with.
				...
				//trap->Traces.  These probably need to be replaced with the more
...
//---------------------------------------------------------
{
	...
	for (i = 0; i < traces; i++ )
	{
		...
		{
			//[WeaponSys]
			trap->Trace( &tr, start, shotMins, shotMaxs, end, skip, MASK_SHOT, qfalse, G2TRFLAG_DOGHOULTRACE|G2TRFLAG_GETSURFINDEX|G2TRFLAG_THICK|G2TRFLAG_HITCORPSES, g_g2TraceLod.integer );
			//[/WeaponSys]
		}
		else
		{
			//[WeaponSys]
			trap->Trace( &tr, start, shotMins, shotMaxs, end, skip, MASK_SHOT, qfalse, 0, 0 );
			//[/WeaponSys]
		}
...
//---------------------------------------------------------
{
	...
	else
	{
		...
		//angs[YAW]	+= Q_flran(-1.0f, 1.0f) * REPEATER_SPREAD;
...
//[DualPistols]
void CalcMuzzlePoint2 ( gentity_t *ent, const vec3_t inForward, const vec3_t inRight, const vec3_t inUp, vec3_t muzzlePoint ) 
{
	int weapontype;
	vec3_t muzzleOffPoint;
