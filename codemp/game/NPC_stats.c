	...
	if ( !md3Model )
	{
		...
		{//set up the playerModel to be the playerModel of the entity with the player script_targetname
			...
			{//no players have spawned in yet.  As such, wing it by using jan
				...
				*s2 = '\0';
				if ( scan != 3 ) {
					Com_Printf( "^3Warning: ojp_spmodelrgb could not parse out all 3 valid colors! Using Defaults.\n" );
					rgb[0] = 255;
					rgb[1] = 255;
					rgb[2] = 255;
				}
			...
			else
			{
				...
				//trap->GetUserinfo( player->client->ps.clientNum, userinfo, sizeof( userinfo ) );
		...
		//[CoOp]
		/* RAFIXME - figure out a way to do this?  Do we even want it?
		...
		if ( surfOff && surfOff[0] )
		{
			...
			while ( 1 ) 
			{
				...
				//turn off this surf
				NPC_SetSurfaceOnOff( NPCS.NPC, token, TURN_OFF );
			}
		}
		if ( surfOn && surfOn[0] )
		{
			...
			while ( 1 )
			{
				...
				//turn on this surf
				NPC_SetSurfaceOnOff( NPCS.NPC, token, TURN_ON );
			}
		}
		*/
		//[/CoOp]
