...
void CheckLMS()
{
	...
	if (!LMSReset //not already reseting
		&& ojp_lastmanstanding.integer > 0 && BG_IsLMSGametype(level.gametype) 
		&& LMS_EnoughPlayers())
...
static void G_Animate ( gentity_t *self )
{
	...
	if ( self->s.frame == self->endFrame )
	{
		if ( self->r.svFlags & SVF_ANIMATING )
		{
			...
			if ( self->ghoul2 )
			{
				...
				trap->G2API_GetBoneAnimIndex( &self->ghoul2[self->playerModel], self->rootBone, 
	...
	if ( self->ghoul2.size())
	{
		...
		trap->G2API_SetBoneAnimIndex( &self->ghoul2[self->playerModel], self->rootBone, 
