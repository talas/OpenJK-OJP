...
{//returns an entity that we can remove to prevent the game from overloading 
	...
	for( i=MAX_CLIENTS; i <level.num_entities;i++)
	{
		...
		{
			trap->Print("Warning, removing instant medpack to prevent entity overflow.\n");
			return e;
		}
	}
	...
	for ( i = MAX_CLIENTS ; i<level.num_entities ; i++, e++) 
	{
		...
		{//found one!
			trap->Print("Warning: FindRemoveAbleGent removed a corpse to prevent a max entity overflow.\n");
			return e;
		}
	}
	...
	for ( i = MAX_CLIENTS ; i<level.num_entities ; i++, e++) 
	{
		...
		{//found one
			trap->Print("Warning: FindRemoveAbleGent removed a dead NPC to prevent a max entity overflow.\n");
			return e;
		}
	}
	...
	for ( i = MAX_CLIENTS ; i<level.num_entities ; i++, e++) 
	{
		...
		{//found one
			trap->Print("Warning: FindRemoveAbleGent removed a NPC that was faking death to prevent a max entity overflow.\n");
			return e;
		}
	}
	...
	for ( i = MAX_CLIENTS ; i<level.num_entities ; i++, e++) 
	{
		...
		{//found one
			trap->Print("Warning: FindRemoveAbleGent removed a light entity to prevent a max entity overflow.\n");
			return e;
		}
	}
...
{	//send request to client to play an effect bolted to a bolt on a ghoul2 entity
	//trap->FX_PlayBoltedEffectID doesn't take inputs for angle or lifetime however :(
	gentity_t	*te = NULL;

	{//If we send this and they don't have the client,
...
void G_UseDispenserOn(gentity_t *ent, int dispType, gentity_t *target)
{
	...
	else if (dispType == HI_AMMODISP)
	{
		...
		{ //do the next increment
			...
			target->bullets[weaponData[target->client->ps.weapon].ammoIndex] += weaponData[target->client->ps.weapon].energyPerShot;

			if (target->bullets[weaponData[target->client->ps.weapon].ammoIndex] > ammoData[weaponData[target->client->ps.weapon].ammoIndex].max)
			{ //cap it off
				target->bullets[weaponData[target->client->ps.weapon].ammoIndex] = ammoData[weaponData[target->client->ps.weapon].ammoIndex].max;
			}
...
int G_CanUseDispOn(gentity_t *ent, int dispType)
{
	...
	else if (dispType == HI_AMMODISP)
	{
		...
		if (ent->bullets[weaponData[ent->client->ps.weapon].ammoIndex] < ammoData[weaponData[ent->client->ps.weapon].ammoIndex].max)
