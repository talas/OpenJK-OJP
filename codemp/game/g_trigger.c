...
void InitTrigger( gentity_t *self ) {
	...
	//[CoOp]
	//only set brush model if there's a model to set.
	if(self->model)
	{
...
void trigger_visible_think(gentity_t *self)
{
	...
	for ( i = 0; i < MAX_CLIENTS; i++ )
	{
		...
		if (self->genericValue1)
		{//FOV check
			if(!InFOV3(self->s.origin, player->client->renderInfo.eyePoint, player->client->ps.viewangles, self->genericValue1, self->genericValue1))
			{
				continue;
			}
		}
