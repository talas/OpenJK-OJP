...
{ // weapon is actually ammotype
	...
	if ( new_count > old_count ) {
		*gaveSome = qtrue;
		...
		if ( ent->client->ps.eFlags & EF_DOUBLE_AMMO ) {
			if ( new_count < ammoData[weapon].max*2 )
				*stop = 0;
		} else {
			if ( new_count < ammoData[weapon].max )
				*stop = 0;
		}
	}
}
...
qboolean G_AmmoDisabled (int wDisable, gitem_t *item)
{
	...
	{
		trap->Print("Error: G_AmmoDisabled run for an non-ammo item.\n"); 
		return qfalse;
	}
	...
	switch ( item->giTag )
	{
		...
		case AMMO_DETPACK:
			...
			break;
		default:
			trap->Print("Error: G_AmmoDisabled couldn't find ammo type.\n");
			return qfalse;
			break;
	};
}
