	...
	*/
			...
			{//hit something
				...
				if ( traceEnt->takedamage 
					...
					&& (!traceEnt->client || traceEnt == NPCS.NPC->enemy || traceEnt->client->NPC_class != NPCS.NPC->client->NPC_class) )
				{//smack
					int dmg = Q_irand( 10, 20 )*(g_npcspskill.integer+1);//NOTE: was 6-12
					//FIXME: debounce?
					//FIXME: do dismemberment
					G_Damage( traceEnt, NPCS.NPC, NPCS.NPC, vec3_origin, trace.endpos, dmg, DAMAGE_NO_KNOCKBACK, MOD_SABER );//MOD_MELEE );
					if ( traceEnt->client )
					{
						if ( !Q_irand( 0, 2 ) )
						{
...
{//Scepter slam attack
	...
	boltIndex = trap->G2API_AddBolt(NPCS.NPC->client->weaponGhoul2[1], 0, "*weapon");
	if ( boltIndex != -1 )
	{
		mdxaBone_t	boltMatrix;
		...
		for ( i = 0; i < numEnts; i++ )
		{
			...
			{//must be a client
				...
				//if ( G_EntIsBreakable( radiusEnt->s.number, NPCS.NPC ) )
				{//damage breakables within range, but not as much
					G_Damage( radiusEnt, NPCS.NPC, NPCS.NPC, vec3_origin, radiusEnt->r.currentOrigin, 100, 0, MOD_ROCKET_SPLASH/*RAFIXME - reimpliment MOD_EXPLOSIVE_SPLASH*/ );
				}
				continue;
			}
			...
			*/
			...
			if ( dist <= radius )
			{
				...
				{//close enough to do damage, too
					G_Damage( radiusEnt, NPCS.NPC, NPCS.NPC, vec3_origin, radiusEnt->r.currentOrigin, Q_irand( 20, 30 ), DAMAGE_NO_KNOCKBACK, MOD_ROCKET_SPLASH/* RAFIXME - reimpliment MOD_EXPLOSIVE_SPLASH*/ );
				}
				if ( radiusEnt->client
					...
					&& radiusEnt->client->NPC_class != CLASS_ATST )
				{
					...
					if ( radiusEnt->health > 0 )
					{
						...
						{//within range of my fist or within ground-shaking range and not in the air
							//[KnockdownSys]
							G_Knockdown( radiusEnt, NPCS.NPC, vec3_origin, 500, qtrue );
...
{//Activate the scepter beam for the current NPC.
	...
	//G_PlayEffect( G_EffectIndex( "scepter/beam_warmup.efx" ), NPCS.NPC->client->weaponGhoul2[1], NPCS.NPC->NPC->genericBolt1, NPCS.NPC->s.number, NPCS.NPC->r.currentOrigin, 0, qtrue );
...
{//Activate the scepter slam for the current NPC.
	...
	//G_PlayEffect( G_EffectIndex( "scepter/slam_warmup.efx" ), NPCS.NPC->weaponModel[1], NPCS.NPC->genericBolt1, NPCS.NPC->s.number, NPCS.NPC->currentOrigin, 0, qtrue );
...
void Tavion_SithSwordRecharge( void )
{
		...
		&& NPCS.NPC->client->weaponGhoul2[0] )
	{
		...
		boltIndex = trap->G2API_AddBolt(NPCS.NPC->client->weaponGhoul2[0], 0, "*weapon");
		NPCS.NPC->client->ps.legsTimer = NPCS.NPC->client->ps.torsoTimer = 0;
		...
		//G_PlayEffect( G_EffectIndex( "scepter/recharge.efx" ), NPCS.NPC->weaponModel[0], boltIndex, NPCS.NPC->s.number, NPCS.NPC->currentOrigin, NPCS.NPC->client->ps.torsoAnimTimer, qtrue );
		NPCS.NPC->painDebounceTime = level.time + NPCS.NPC->client->ps.torsoTimer;
		NPCS.NPC->client->ps.pm_time = NPCS.NPC->client->ps.torsoTimer;
		NPCS.NPC->client->ps.pm_flags |= PMF_TIME_KNOCKBACK;
		VectorClear( NPCS.NPC->client->ps.velocity );
		...
		//G_PlayEffect( G_EffectIndex( "scepter/invincibility.efx" ), NPCS.NPC->playerModel, 0, NPCS.NPC->s.number, NPCS.NPC->currentOrigin, NPCS.NPC->client->ps.torsoAnimTimer + 10000, qfalse );
...
float G_GroundDistance( gentity_t *self )
{
	...
	trap->Trace(&tr, self->client->ps.origin, self->r.mins, self->r.maxs, down, self->s.clientNum, MASK_SOLID, qfalse, 0, 0);
...
static void Jedi_CheckDecreaseSaberAnimLevel( void )
{
	...
	{//not attacking
		if ( TIMER_Done( NPCS.NPC, "saberLevelDebounce" ) && !Q_irand( 0, 10 ) )
		{
			...
			//[StanceSelection]
			Jedi_AdjustSaberAnimLevel( NPCS.NPC, Q_irand( SS_FAST, SS_STAFF ));//random
			//[/StanceSelection]
...
{//check to see if we feel like kicking
	...
	if ( NPCS.NPC->client->ps.weapon == WP_SABER )
	{
		...
		else if ( NPCS.NPC->client->saber[1].model 
...
void Kyle_GrabEnemy( void )
{
	...
	//WP_SabersCheckLock2( NPCS.NPC, NPCS.NPC->enemy, Q_irand(LOCK_KYLE_GRAB1,LOCK_KYLE_GRAB2) );//LOCK_KYLE_GRAB3
...
{//have the kyle boss NPC attempt a grab.
	...
	NPCS.NPC->client->ps.saberMove /* not sure what this is = NPCS.NPC->client->ps.saberMoveNext*/ = LS_READY;
	VectorClear( NPCS.NPC->client->ps.velocity );
	VectorClear( NPCS.NPC->client->ps.moveDir );
...
{//Kyle can do grab on this enemy
	if ( NPCS.NPC->client->NPC_class == CLASS_KYLE && (NPCS.NPC->spawnflags&1) )
	{//Boss Kyle
					...
					if ( !PM_InOnGroundAnim( NPCS.NPC->enemy->client->ps.legsAnim ) )
					{
							...
							&& !NPCS.NPC->client->ps.saberInFlight )
						{
							...
							{//close to same level of ground
								if ( DistanceSquared( NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin ) <= 10000.0f )
								{
									return qtrue;
								}
							}
...
{//FIXME: for many of these checks, what we really want is horizontal distance to enemy
		...
		&& NPCS.NPC->enemy->s.apos.trType == TR_STATIONARY )
	{
		...
		{//close enough to push
			if ( InFront( NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->client->renderInfo.eyePoint, NPCS.NPC->client->renderInfo.eyeAngles, 0.6f ) )
			{//knock it down
				//RAFIXME impliment this function
				//WP_KnockdownTurret( NPCS.NPC, NPCS.NPC->enemy );
	...
	{//can't use saber and they're in striking range
		if ( !Q_irand( 0, 5 ) && InFront( NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin, NPCS.NPC->client->ps.viewangles, 0.2f ) )
		{
			if ( ((NPCS.NPCInfo->scriptFlags&SCF_DONT_FIRE)||NPCS.NPC->client->pers.maxHealth - NPCS.NPC->health > NPCS.NPC->client->pers.maxHealth*0.25f)//lost over 1/4 of our health or not firing
				//[CoOp]
				&& WP_ForcePowerUsable( NPCS.NPC, FP_DRAIN )//know how to drain
				&& WP_ForcePowerAvailable( NPCS.NPC, FP_DRAIN, 20 )//and have enough power
				//[/CoOp]
				&& !Q_irand( 0, 2 ) )
			{//drain
			...
			{//ok, we can't drain at the moment
				...
				{//let's try a kick
					if ( G_PickAutoMultiKick( NPCS.NPC, qfalse, qtrue ) != LS_NONE 
						|| (G_CanKickEntity(NPCS.NPC, NPCS.NPC->enemy) && G_PickAutoKick( NPCS.NPC, NPCS.NPC->enemy, qtrue ) != LS_NONE) ) 
					{//kicked!
	...
	{//we're too damn close!
		...
		else if ( NPCS.NPC->client->ps.stats[STAT_WEAPONS]&(1<<WP_SCEPTER) 
			&& !Q_irand( 0, 20 ) )
		{
	...
	{//we're out of striking range and we are allowed to attack
		...
		{//They're being gripped, rush them!
			...
			if ( (NPCS.NPCInfo->rank >= RANK_LT_JG||WP_ForcePowerUsable( NPCS.NPC, FP_SABERTHROW ))
			//[/CoOp]
				&& !Q_irand( 0, 5 )
				&& !(NPCS.NPC->client->ps.fd.forcePowersActive&(1 << FP_SPEED))
				&& !(NPCS.NPC->client->ps.saberEventFlags&SEF_INWATER) )//saber not in water
			...
			&& InFOV3( NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin, NPCS.NPC->client->ps.viewangles, 20, 30 ) )
			//[/CoOp]
				...
				{//else, randomly try some kind of attack every now and then
						...
						&& (!Q_irand( 0, 1 ) || NPCS.NPC->s.weapon != WP_SABER) )
					{
						if ( WP_ForcePowerUsable( NPCS.NPC, FP_PULL ) && !Q_irand( 0, 2 ) )
						{
							...
							//FIXME: check forcePushRadius[NPCS.NPC->client->ps.fd.forcePowerLevel[FP_PUSH]]
							ForceThrow( NPCS.NPC, qtrue );
							//maybe strafe too?
						...
						else if ( WP_ForcePowerUsable( NPCS.NPC, FP_LIGHTNING ) && (( (NPCS.NPCInfo->scriptFlags & SCF_DONT_FIRE) && Q_stricmp( "cultist_lightning", NPCS.NPC->NPC_type )) || Q_irand( 0, 1 ) ) )
						{
						...
						//else if ( WP_ForcePowerAvailable( NPCS.NPC, FP_GRIP, 0 ) )
						{
							//taunt
							if ( TIMER_Done( NPCS.NPC, "chatter" ) && jediSpeechDebounceTime[NPCS.NPC->client->playerTeam] < level.time && NPCS.NPCInfo->blockedSpeechDebounceTime < level.time )
							{
	...
	else 
	{
		...
		{//jump!
			...
			{//gotta jump!
				...
				{
					if ( g_slowmoDuelEnd.integer > 5 && self->enemy && !self->enemy->s.number )
					{
...
{//evade special attacks
	...
	{//back away from these
		...
		{//see if we should back off
			...
			{//facing me
				float minSafeDistSq = (NPCS.NPC->r.maxs[0]*1.5f+NPCS.NPC->enemy->r.maxs[0]*1.5f+WP_SaberLength(NPCS.NPC->enemy)+24.0f);
	...
	{//check some other attacks?
		...
		if ( NPCS.NPC->enemy->client->ps.torsoAnim == BOTH_ROLL_STAB
			|| (NPCS.NPC->enemy->client->ps.torsoAnim == BOTH_ROLL_F && ((NPCS.NPCInfo->last_ucmd.buttons&BUTTON_ATTACK)
			/* RAFIXME:  is this implimented? ||(NPCS.NPC->enemy->client->ps.pm_flags&PMF_ATTACK_HELD)*/) ) )
		{//either already in a roll-stab or may go into one
			...
			{//see if we should evade
				...
				{//facing me
					...
					float minSafeDistSq = (NPCS.NPC->r.maxs[0]*1.5f+NPCS.NPC->enemy->r.maxs[0]*1.5f+WP_SaberLength(NPCS.NPC->enemy)+24.0f);
					minSafeDistSq *= minSafeDistSq;
					distSq = DistanceSquared( NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.currentOrigin );
					if ( distSq < minSafeDistSq )
					{//evade!
						qboolean doJump = ( NPCS.NPC->enemy->client->ps.torsoAnim == BOTH_ROLL_STAB || distSq < 3000.0f );//not much time left, just jump!
						...
						{//roll?
							...
							{//I'm to his right
								...
								{//roll to the left.
									...
									{//fuck it, just force it
										NPC_SetAnim(NPCS.NPC,SETANIM_BOTH,BOTH_ROLL_L,SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
							...
							{//I'm to his left
								...
								{//roll to the right
									...
									if ( NPCS.NPC->client->ps.groundEntityNum != ENTITYNUM_NONE )
									{//fuck it, just force it
										NPC_SetAnim(NPCS.NPC,SETANIM_BOTH,BOTH_ROLL_R,SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
										G_AddEvent( NPCS.NPC, EV_ROLL, 0 );
										NPCS.NPC->client->ps.saberMove = LS_NONE;
									}
								}
...
//[/CoOp]
{
	...
	*/
	...
	{
		for ( bladeNum = 0; bladeNum < NPCS.NPC->enemy->client->saber[saberNum].numBlades; bladeNum++ )
		{
			...
			{//valid saber and this blade is on
				...
				dist = ShortestLineSegBewteen2LineSegs( NPCS.NPC->enemy->client->saber[saberNum].blade[bladeNum].muzzlePoint, saberTip, bottom, top, saberPoint, axisPoint );
				if ( dist < bestDist )
				{
					bestDist = dist;
	...
	if ( bestDist > NPCS.NPC->r.maxs[0]*5 )//was *3
	{//FIXME: sometimes he reacts when you're too far away to actually hit him
		if ( d_JediAI.integer )
		{
	...
	VectorMA( NPCS.NPC->enemy->client->saber[closestSaberNum].blade[closestBladeNum].muzzlePointOld, NPCS.NPC->enemy->client->saber[closestSaberNum].blade[closestBladeNum].length, NPCS.NPC->enemy->client->saber[closestSaberNum].blade[closestBladeNum].muzzleDirOld, saberTipOld );
	VectorMA( NPCS.NPC->enemy->client->saber[closestSaberNum].blade[closestBladeNum].muzzlePoint, NPCS.NPC->enemy->client->saber[closestSaberNum].blade[closestBladeNum].length, NPCS.NPC->enemy->client->saber[closestSaberNum].blade[closestBladeNum].muzzleDir, saberTip );

	//VectorMA( NPCS.NPC->enemy->client->saber[saberNum].blade[bladeNum].muzzlePointOld, NPCS.NPC->enemy->client->saber[saberNum].blade[bladeNum].length, NPCS.NPC->enemy->client->saber[saberNum].blade[bladeNum].muzzleDirOld, saberTipOld );
	//VectorMA( NPCS.NPC->enemy->client->saber[saberNum].blade[bladeNum].muzzlePoint, NPCS.NPC->enemy->client->saber[saberNum].blade[bladeNum].length, NPCS.NPC->enemy->client->saber[saberNum].blade[bladeNum].muzzleDir, saberTip );
	//[/CoOp]
//	VectorCopy(NPCS.NPC->enemy->client->lastSaberBase_Always, muzzlePoint);
//	VectorMA(muzzlePoint, GAME_SABER_LENGTH, NPCS.NPC->enemy->client->lastSaberDir_Always, saberTip);
//	VectorCopy(saberTip, saberTipOld);
	...
	dist = ShortestLineSegBewteen2LineSegs( 
		NPCS.NPC->enemy->client->saber[closestSaberNum].blade[closestBladeNum].muzzlePoint, 
	...
	VectorSubtract( 
		NPCS.NPC->enemy->client->saber[closestSaberNum].blade[closestBladeNum].muzzlePoint, 
		NPCS.NPC->enemy->client->saber[closestSaberNum].blade[closestBladeNum].muzzlePointOld, 
		baseDir );
	//[/CoOp]
	...
	//[CoOp]
	if ( (evasionType=Jedi_SaberBlockGo( NPCS.NPC, &NPCS.ucmd, hitloc, dir, NULL, dist )) != EVASION_NONE )
	...
	{//check to see if he's coming at me
		...
		{//coming at/facing me!
			...
			case 3:
				...
				else if ( (NPCS.NPCInfo->rank == RANK_ENSIGN || NPCS.NPCInfo->rank > RANK_LT_JG) && TIMER_Done( NPCS.NPC, "parryTime" ) )
				//[/CoOp]
				{//FIXME: check forcePushRadius[NPCS.NPC->client->ps.fd.forcePowerLevel[FP_PUSH]]
					ForceThrow( NPCS.NPC, qfalse );
				}
...
static void Jedi_CombatTimersUpdate( int enemy_dist )
{
	...
	{//some kind of saber combat event is still pending
		...
		{//parried
			...
			*/
			//[CoOp]
			if ( NPCS.NPC->enemy && (!NPCS.NPC->enemy->client||PM_SaberInKnockaway( NPCS.NPC->enemy->client->ps.saberMove )) )
			//[/CoOp]
			{//advance!
...
static void Jedi_CheckEnemyMovement( float enemy_dist )
{
	...
	{//we're not a boss
		//[CoOp]
		if ( BG_KickingAnim( NPCS.NPC->enemy->client->ps.legsAnim )
	...
	{//racc - we're not holding back
		...
		{//not gripping
			...
			{//hunt him down
				...
				if ( (NPC_ClearLOS4( NPCS.NPC->enemy )||NPCS.NPCInfo->enemyLastSeenTime>level.time-500) && NPC_FaceEnemy( qtrue ) )//( NPCInfo->rank == RANK_CREWMAN || NPCS.NPCInfo->rank > RANK_LT_JG ) && 
				{
					//try to jump to him?
					/*
					vec3_t end;
					...
					*/
					...
					if (NPCS.NPC->client && NPCS.NPC->client->NPC_class==CLASS_BOBAFETT)
					{
						Boba_FireDecide();
					}
					//[/CoOp]
			...
			{//have one to consider
				...
				{//we have a clear (of architecture) LOS to him
					...
					{//just get mad right away
						if ( DistanceHorizontalSquared( NPCS.NPC->currentOrigin, best_enemy->currentOrigin ) < (1024*1024) )
						{
...
{//decide if we want to do a kata and do it
	if ( NPCS.NPCInfo->rank >= RANK_LT_COMM )
				...
				{//on the ground
					...
					{//not going to try to jump
						...
						{//uh-oh, no jumping moves!
							if ( NPCS.NPC->client->ps.saberAnimLevel == SS_STAFF )
...
static void Jedi_Attack( void )
{
	...
	if ( NPCS.NPC->enemy )
	{
		...
		if ( NPCS.NPC->enemy->health <= 0 && NPCS.NPC->enemy->enemy == NPCS.NPC && (NPCS.NPC->client->playerTeam != NPCTEAM_PLAYER ||
			(NPCS.NPC->client->NPC_class == CLASS_KYLE
			&& (NPCS.NPC->spawnflags&1) && NPCS.NPC->s.number < MAX_CLIENTS)) )//good guys don't gloat
		//[/CoOp]
		...
		if ( NPCS.NPC->client->NPC_class == CLASS_ALORA )
		{
			...
			{//chance of doing a special dual saber throw
				...
				{//has dual sabers and haven't already tossed the saber.
					...
					{
						NPC_SetAnim( NPCS.NPC, SETANIM_BOTH, BOTH_ALORA_SPIN_THROW, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
						NPCS.NPC->client->ps.weaponTime = NPCS.NPC->client->ps.torsoTimer;
						//FIXME: don't move
				...
				&& NPCS.ucmd.forwardmove > 0 
				&& fabs((float)NPCS.ucmd.rightmove) < 32
				&& !(NPCS.ucmd.buttons&BUTTON_WALKING)
				&& !(NPCS.ucmd.buttons&BUTTON_ATTACK)
				...
				if ( Distance( NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin ) > 80 )
				{
						...
						|| NPCS.NPC->client->ps.legsAnim == BOTH_ALORA_FLIP_3 )
					{
						...
						{//go ahead and start anotther
							NPC_SetAnim( NPCS.NPC, SETANIM_BOTH, Q_irand(BOTH_ALORA_FLIP_1,BOTH_ALORA_FLIP_3), SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
						}
					}
					else if ( !Q_irand( 0, 6 ) )
					{
...
qboolean Jedi_InSpecialMove( void )
{
	...
	{//going into the scepter laser attack
		...
		{//go into the hold
			...
			//RAFIXME - impliment
			//G_PlayEffect( G_EffectIndex( "scepter/beam.efx" ), NPCS.NPC->client->weaponGhoul2[1], NPCS.NPC->NPC->genericBolt1, NPCS.NPC->s.number, NPCS.NPC->r.currentOrigin, 10000, qtrue );
			NPCS.NPC->client->ps.legsTimer = NPCS.NPC->client->ps.torsoTimer = 0;
	...
	else if ( NPCS.NPC->client->ps.torsoAnim == BOTH_SCEPTER_HOLD )
	{
		if ( NPCS.NPC->client->ps.torsoTimer <= 100 )
		{
			NPCS.NPC->s.loopSound = 0;
			//RAFIXME - impliment
			//G_StopEffect( G_EffectIndex( "scepter/beam.efx" ), NPCS.NPC->client->weaponGhoul2[1], NPCS.NPC->NPC->genericBolt1, NPCS.NPC->s.number );
			NPCS.NPC->client->ps.legsTimer = NPCS.NPC->client->ps.torsoTimer = 0;
	...
	else if ( NPCS.NPC->client->ps.torsoAnim == BOTH_SCEPTER_STOP )
	{
		if ( NPCS.NPC->enemy )
		{
			NPC_FaceEnemy( qtrue );
	...
	else if ( NPCS.NPC->client->ps.torsoAnim == BOTH_TAVION_SCEPTERGROUND )
	{
		...
		{//NPC heals Rosh
			...
			{//have our heal target
				...
				*/
				...
				/*
				if ( !helpingRosh 
					&& !TIMER_Done( NPCS.NPC->client->leader, "chargeMeUp" ) 
					&& NPCS.NPC->client->leader->health > 0)
				{
					Kothos_PowerRosh();
					helpingRosh = qtrue;
				}
				*/
				...
				else if ( NPCS.NPC->enemy && DistanceSquared( NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin ) < Twins_DangerDist() )
				{//enemies to attack and close enough to our heal target.
					...
					{//not blocked in movement
						...
						{//attack!
							if ( NPCS.NPC->painDebounceTime > level.time  //just get hurt
								|| (NPCS.NPC->health < 100 && Q_irand(-20, (g_npcspskill.integer+1)*10) > 0 ) //injured
								|| !Q_irand( 0, 80-(g_npcspskill.integer*20) ) )//just randomly
							{
								...
								case 3:
									//Push the bastard.
									ForceThrow( NPCS.NPC, qfalse );
									NPCS.NPC->client->ps.weaponTime = Q_irand( 1000, 3000 )+(2-g_npcspskill.integer)*1000;
									if ( NPCS.NPC->painDebounceTime <= level.time 
										&& NPCS.NPC->health >= 100 )
								...
								case 5:
									...
									//that the "draining" timer can handle it.
									//ForceDrain2( NPCS.NPC );
									NPCS.NPC->client->ps.weaponTime = Q_irand( 3000, 6000 )+(2-g_npcspskill.integer)*2000;
									TIMER_Set( NPCS.NPC, "draining", NPCS.NPC->client->ps.weaponTime );
									if ( NPCS.NPC->painDebounceTime <= level.time 
										&& NPCS.NPC->health >= 100 )
									{//have an attack delay if this attack isn't the result
								...
								case 7:
									//try to grip them
									if ( NPCS.NPC->enemy && InFOV3( NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin, NPCS.NPC->client->ps.viewangles, 20, 30 ) )
									{
										NPCS.NPC->client->ps.weaponTime = Q_irand( 3000, 6000 )+(2-g_npcspskill.integer)*2000;
										TIMER_Set( NPCS.NPC, "gripping", 3000 );
										if ( NPCS.NPC->painDebounceTime <= level.time 
					...
					|| DistanceSquared( NPCS.NPC->r.currentOrigin, NPCS.NPC->client->leader->r.currentOrigin ) > (512*512) )
		...
		{//I'm rosh!
			...
			{//Vil and/or Dasariah still around to heal me
				if ( NPCS.NPC->health == 1 //need healing
					|| NPCS.NPC->flags&FL_GODMODE )//being healed
				{//FIXME: custom anims
					...
					{//twins are still alive
						...
						{//not attacking
							...
							{//get down and wait for Vil or Dasariah to help us
								//FIXME: sound?
								NPCS.NPC->client->ps.legsTimer = NPCS.NPC->client->ps.torsoTimer = 0;
								NPC_SetAnim( NPCS.NPC, SETANIM_BOTH, BOTH_FORCEHEAL_START, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
								NPCS.NPC->client->ps.torsoTimer = NPCS.NPC->client->ps.legsTimer = -1; //hold the animation
						...
						NPCS.NPC->client->ps.saberBlocked = BLOCKED_NONE;
						NPCS.NPC->client->ps.saberMove /* not sure what this is = NPCS.NPC->client->ps.saberMoveNext*/ = LS_NONE;
	...
	if ( BG_SuperBreakWinAnim( NPCS.NPC->client->ps.torsoAnim ) )
...
{//this NPC attempts to heal the heal target (the rosh boss normally).
		...
		&& NPCS.NPC->client->leader->client )
	{
		if ( DistanceSquared( NPCS.NPC->client->leader->r.currentOrigin, NPCS.NPC->r.currentOrigin ) <= (256*256)
			&& G_ClearLineOfSight( NPCS.NPC->client->leader->client->renderInfo.eyePoint, NPCS.NPC->client->renderInfo.eyePoint, NPCS.NPC->s.number, MASK_OPAQUE ) )
		{
			NPC_SetAnim( NPCS.NPC, SETANIM_TORSO, BOTH_FORCE_2HANDEDLIGHTNING_HOLD, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
			...
			//gentity_t *tent = G_TempEntity( NPCS.NPC->r.currentOrigin, EV_KOTHOS_BEAM );
			//tent->svFlags |= SVF_BROADCAST;
			//tent->s.otherEntityNum = NPCS.NPC->s.number;
			//tent->s.otherEntityNum2 = NPCS.NPC->client->leader->s.number;
			//END HACK LINE

			NPCS.NPC->client->leader->health += Q_irand( 1+g_npcspskill.integer*2, 4+g_npcspskill.integer*3 );//from 1-5 to 4-10
			if ( NPCS.NPC->client->leader->client )
			{
				if ( NPCS.NPC->client->leader->client->ps.legsAnim == BOTH_FORCEHEAL_START 
					&& NPCS.NPC->client->leader->health >= NPCS.NPC->client->leader->client->pers.maxHealth )
				{//let him get up now
					NPC_SetAnim( NPCS.NPC->client->leader, SETANIM_BOTH, BOTH_FORCEHEAL_STOP, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
					//FIXME: temp effect, effect over?
					//racc - impliment this effect somehow.
					//G_PlayEffect( G_EffectIndex( "force/kothos_recharge.efx" ), 0, 0, NPCS.NPC->client->leader->s.number, NPCS.NPC->client->leader->r.currentOrigin, NPCS.NPC->client->leader->client->ps.torsoTimer, qfalse );
				...
				{//start effect?
					//racc - impliment this effect somehow.
					//G_PlayEffect( G_EffectIndex( "force/kothos_recharge.efx" ), 0, 0, NPCS.NPC->client->leader->s.number, NPCS.NPC->client->leader->r.currentOrigin, 500, qfalse );
					NPCS.NPC->client->leader->flags |= FL_GODMODE;
			...
			{//not on easy
				...
				//G_PlayEffect( G_EffectIndex( "force/kothos_recharge.efx" ), 0, 0, NPCS.NPC->s.number, NPCS.NPC->r.currentOrigin, 500, qfalse );
				NPCS.NPC->client->leader->flags |= FL_GODMODE;
...
void Kothos_PowerRosh( void )
{
		...
		&& NPCS.NPC->client->leader )
	{
			...
			&& G_ClearLineOfSight( NPCS.NPC->client->leader->client->renderInfo.eyePoint, NPCS.NPC->client->renderInfo.eyePoint, NPCS.NPC->s.number, MASK_OPAQUE ) )
		{
			...
			//RAFIXME - impliment this effect function.
			//G_PlayEffect( G_EffectIndex( "force/kothos_beam.efx" ), 0, 1, NPCS.NPC->s.number, NPCS.NPC->r.currentOrigin, 500, qfalse );
			if ( NPCS.NPC->client->leader->client )
...
{//racc - I think this is suppose to be the movement code for the Kothos healers when
	...
	/*
	STEER::Activate( NPCS.NPC );
	STEER::Evade( NPCS.NPC, NPCS.NPC->enemy );
	STEER::AvoidCollisions( NPCS.NPC, NPCS.NPC->client->leader );
	STEER::DeActivate( NPCS.NPC, &NPCS.ucmd );
	*/
	if ( (NPCS.NPCInfo->aiFlags&NPCAI_BLOCKED) )
	{
