...
//[DualPistols]
static QINLINE animNumber_t PM_GetWeaponReadyAnim(void)
{
...
#ifdef _GAME
		if( pm->waterlevel == 0 && !pm->noFootsteps)
...
void PM_FinishWeaponChange( void ) {
	...
	{// This looks wrong if there is no saber present, so pretend we're force pulling it back
		...
		{
			PM_SetAnim(SETANIM_TORSO, BOTH_SABERPULL, SETANIM_FLAG_OVERRIDE);
		}
	}
	*/
...
void PM_DoPunch(void)
{
	...
	PM_SetAnim(SETANIM_TORSO, desTAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD|SETANIM_FLAG_HOLDLESS);
...
void PmoveSingle (pmove_t *pmove) {
	...
	{
		stiffenedUp = qtrue;
	//PM_SetAnim( SETANIM_BOTH, BOTH_STAND1IDLE1, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
