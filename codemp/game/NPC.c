...
*/
	...
	if( NPCS.NPCInfo->scriptFlags & SCF_SAFE_REMOVE )
	{
		...
		//if( !gi.inPVS( NPCS.NPC->currentOrigin, g_entities[0].currentOrigin ) )//FIXME: use cg.vieworg?
		{//racc - we can be safely removed.
			G_UseTargets2( NPCS.NPC, NPCS.NPC, NPCS.NPC->target3 );
			NPCS.NPC->s.eFlags |= EF_NODRAW;
			//NPCS.NPC->r.svFlags &= ~SVF_NPC; //RAFIXME - impliment this flag?
...
void NPC_HandleAIFlags (void)
{
	...
	//----------------------------
	if (NPCS.NPCInfo->scriptFlags & SCF_FLY_WITH_JET)
	{
		qboolean	ShouldFly  = !!(NPCS.NPCInfo->aiFlags & NPCAI_FLY);
		qboolean	IsFlying   = !!(JET_Flying(NPC));
		qboolean	IsInTheAir = (NPCS.NPC->client->ps.groundEntityNum==ENTITYNUM_NONE);
		...
		if (IsFlying)
		{
			...
			if (IsInTheAir)
			{
				...
				ground[2] -= 60.0f;
                trap->Trace(&trace, NPCS.NPC->r.currentOrigin, 0, 0, ground, NPCS.NPC->s.number, NPCS.NPC->clipmask);
			...
			if (IsInTheAir)
			{
				NPCS.NPC->lastInAirTime = level.time;
				ShouldFly = qtrue;
			}
			...
			//-----------------------------------------------------
			else if (!ShouldFly && (level.time - NPCS.NPC->lastInAirTime)>3000)
			{
				NPCS.NPCInfo->aiFlags &= ~NPCAI_FLY;
			}
		}
		...
		{
			JET_FlyStart(NPCS.NPC);				// EVENTUALLY, Remove All Other Calls
		}
		...
		{
			JET_FlyStop(NPCS.NPC);				// EVENTUALLY, Remove All Other Calls
		}
	}
	*/
	//[/CoOp]
...
void NPC_RunBehavior( int team, int bState )
{
	...
	*/
	...
	else if ( (NPCS.NPCInfo->scriptFlags&SCF_PILOT) && Pilot_MasterUpdate() )
	{
		return;
	}
	*/
	...
	*/
	...
	else if ( NPCS.NPC->client->ps.weapon == WP_TUSKEN_RIFLE )
	{
	...
	//	case NPCTEAM_MALON:
		...
		case NPCTEAM_ENEMY:
			...
			*/
			...
			//[CoOp]
			if ( NPCS.NPC->client->ps.weapon == WP_THERMAL || NPCS.NPC->client->ps.weapon == WP_MELEE )//FIXME: separate AI for melee fighters
			//[/CoOp]
			{//a grenadier
		...
		case NPCTEAM_NEUTRAL:
			...
			else if ( NPCS.NPC->client->NPC_class == CLASS_UGNAUGHT 
...
void NPC_Think ( gentity_t *self)//, int msec )
{
	...
	if ( d_npcfreeze.integer || (NPCS.NPC->r.svFlags&SVF_ICARUS_FREEZE) ) 
	{
		NPC_UpdateAngles( qtrue, qtrue );
		ClientThink(self->s.number, &NPCS.ucmd);
		//VectorCopy(self->s.origin, self->s.origin2 );
	...
	{//being controlled by player
		...
		ClientThink( NPCS.NPC->s.number, &ucmd );
		VectorCopy(self->s.origin, self->s.origin2 );
		return;
	}
	*/
	...
	else
	{
		if ( NPCS.NPC->client 
			...
			&& NPCS.NPC->client->ps.fd.forceGripBeingGripped > level.time  //SP version - && (NPC->client->ps.eFlags&EF_FORCE_GRIPPED)
			&& NPCS.NPC->client->ps.eFlags2&EF2_FLYING	//SP version - && NPC->client->moveType == MT_FLYSWIM
