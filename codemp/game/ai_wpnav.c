...
int SavePathData(const char *filename)
{
	...
	G_Printf("Saving data to waypoint file..\n");
...
int AcceptBotCommand(char *cmd, gentity_t *pl)
{
	...
	if (Q_stricmp (cmd, "bot_wp_cmdlist") == 0) //lists all the bot waypoint commands.
	{
		...
		trap_SendServerCommand( bot_wp_editornumber.integer, va("print \"%sbot_wp_add%s - Add a waypoint (optional int parameter will insert the point after the specified waypoint index in a trail)\n\n", S_COLOR_YELLOW, S_COLOR_WHITE));
		trap_SendServerCommand( bot_wp_editornumber.integer, va("print \"%sbot_wp_rem%s - Remove a waypoint (removes last unless waypoint index is specified as a parameter)\n\n", S_COLOR_YELLOW, S_COLOR_WHITE));
		trap_SendServerCommand( bot_wp_editornumber.integer, va("print \"%sbot_wp_addflagged%s - Same as wp_add, but adds a flagged point (type bot_wp_addflagged for help)\n\n", S_COLOR_YELLOW, S_COLOR_WHITE));
		trap_SendServerCommand( bot_wp_editornumber.integer, va("print \"%sbot_wp_switchflags%s - Switches flags on an existing waypoint (type bot_wp_switchflags for help)\n\n", S_COLOR_YELLOW, S_COLOR_WHITE));
