...
void BotWaypointRender(void)
{
	...
	//[BotTweaks]
	//update the bot editor number so we know who to send this stuff to.	
	trap->Cvar_Update(&bot_wp_editornumber);

	if(bot_wp_editornumber.integer < 0)
	{//noone is currently set to be the editor.  Don't render anything.
		return;
	}
	//[/BotTweaks]
	...
	while (i < gWPNum)
	{
		if (gWPArray[i] && gWPArray[i]->inuse)
		{
			...
			plum->s.otherEntityNum = bot_wp_editornumber.integer;
			//[/BotTweaks]
...
//[DynamicMemoryTweaks]
//setting a definition for this to simplify things
...
int SavePathData(const char *filename)
{
	...
	trap->Print("Saving data to waypoint file..\n");
	...
	trap->SendServerCommand( bot_wp_editornumber.integer, va("print \"%sPath data has been saved and updated. You may need to restart the level for some things to be properly calculated.\n", S_COLOR_YELLOW));		
	//[/BotTweaks]
...
int AcceptBotCommand(char *cmd, gentity_t *pl)
{
	...
	//[BotTweaks]
	//only the current active bot_wp_editornumber.integer player can use the edit commands.
	//this is to prevent cheating, which I can see happening if I'm editing the waypoints
	//during matches online.
	trap->Cvar_Update(&bot_wp_editornumber);
	...
	{
		//[BotTweaks]
		//transmit the message to the current waypoint editor client.
		trap->SendServerCommand( bot_wp_editornumber.integer, va("print \"%sbot_wp_add%s - Add a waypoint (optional int parameter will insert the point after the specified waypoint index in a trail)\n\n", S_COLOR_YELLOW, S_COLOR_WHITE));
		trap->SendServerCommand( bot_wp_editornumber.integer, va("print \"%sbot_wp_rem%s - Remove a waypoint (removes last unless waypoint index is specified as a parameter)\n\n", S_COLOR_YELLOW, S_COLOR_WHITE));
		trap->SendServerCommand( bot_wp_editornumber.integer, va("print \"%sbot_wp_addflagged%s - Same as wp_add, but adds a flagged point (type bot_wp_addflagged for help)\n\n", S_COLOR_YELLOW, S_COLOR_WHITE));
		trap->SendServerCommand( bot_wp_editornumber.integer, va("print \"%sbot_wp_switchflags%s - Switches flags on an existing waypoint (type bot_wp_switchflags for help)\n\n", S_COLOR_YELLOW, S_COLOR_WHITE));
		trap->SendServerCommand( bot_wp_editornumber.integer, va("print \"%sbot_wp_tele%s - Teleport yourself to the specified waypoint's location\n\n", S_COLOR_YELLOW, S_COLOR_WHITE));
		trap->SendServerCommand( bot_wp_editornumber.integer, va("print \"%sbot_wp_killoneways%s - Removes oneway (backward and forward) flags on all waypoints in the level\n\n", S_COLOR_YELLOW, S_COLOR_WHITE));
		trap->SendServerCommand( bot_wp_editornumber.integer, va("print \"%sbot_wp_save%s - Saves all waypoint data into a file for later use\n", S_COLOR_YELLOW, S_COLOR_WHITE));
		//[/BotTweaks]
	...
	if (Q_stricmp (cmd, "bot_wp_tele") == 0)
	{
		...
		{	//[BotTweaks]
			//send bot editor mesages to the approprate client.
			trap->SendServerCommand( bot_wp_editornumber.integer, va("print \"%sYou didn't specify an index. Assuming last.\n", S_COLOR_YELLOW));
			//[/BotTweaks]
	...
	if (Q_stricmp (cmd, "bot_wp_switchflags") == 0)
	{
		...
		else
		{
			//[BotTweaks]
			//send bot editor mesages to the approprate client.
			trap->SendServerCommand( bot_wp_editornumber.integer, va("print \"%sWaypoint number (to modify) needed for bot_wp_switchflags\nSyntax: bot_wp_switchflags <flags> <n>\n", S_COLOR_YELLOW));
			//[/BotTweaks]
		}
		return 1;
	}
