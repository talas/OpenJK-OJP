...
//
////////////////////////////////////////////////////////////////////////////////////////
//#include "g_headers.h" //racc - this is a dummy file header NUAM
...
//RAFIXME - this some redefinitions/externs to fix compiler error.  This should be fixed
#define	WAYPOINT_NONE	-1


extern int NAVDEBUG_showNearest;
extern int NAVDEBUG_showTestPath;
...
extern "C"
{
	...
	//NAVDEBUG variables
	extern int NAVDEBUG_showRadius;
	extern int NAVDEBUG_showCollision;
	extern int NAVDEBUG_showCombatPoints;
	extern int NAVDEBUG_showEdges;
	extern int NAVDEBUG_showEnemyPath;
	extern int NAVDEBUG_showNavGoals;
	extern int NAVDEBUG_showNodes;

	//NPC Jump functions
	extern qboolean NPC_TryJump3(const vec3_t pos,	float max_xy_dist, float max_z_diff);
	extern qboolean NPC_TryJump2(gentity_t *goal, float max_xy_dist, float max_z_diff);

	extern void G_DrawEdge( vec3_t start, vec3_t end, int type );
	extern void G_DrawNode( vec3_t origin, int type );
	extern void G_DrawCombatPoint( vec3_t origin, int type );
	extern void G_DrawRadius( vec3_t origin, unsigned int radius, int type );
}
...
////////////////////////////////////////////////////////////////////////////////////////
/*
bool	HFILEopen_read(int& handle,	const char* filepath)		{trap_FS_FOpenFile(filepath, &handle, FS_READ);	return (handle!=0);}
bool	HFILEopen_write(int& handle, const char* filepath)		{trap_FS_FOpenFile(filepath, &handle, FS_WRITE);	return (handle!=0);}
bool	HFILEread(int& handle,		void*		data, int size)	{return (trap_FS_Read(data, size, handle)!=0);}
bool	HFILEwrite(int& handle,		const void* data, int size)	{return (trap_FS_Write(data, size, handle)!=0);}
bool	HFILEclose(int& handle)									{trap_FS_FCloseFile(handle); return true;}
*/
...
//MP uses a different struct for the cvars
extern "C"
{
	extern vmCvar_t		g_nav1;
	extern vmCvar_t		g_nav2;
...
*/
//RAFIXME - impliment delayedShutDown?
/*
extern "C"
{
...
namespace NAV
{
	enum
	{
		...
		CELL_RANGE			= 1000,  //racc - the distance at which nodes must be to each other to be part of the same
									//cell.
		VIEW_RANGE			= 550,
...
public:
	...
	enum EWayNodeFlags
	{
		WN_NONE = 0,
		WN_ISLAND,
		WN_FLOATING,	//racc - this waypoint is in the air.
		WN_DROPTOFLOOR,
		WN_NOAUTOCONNECT,
		WN_MAX
	};
...
public:
	...
	enum EWayEdgeFlags
	{
		...
		WE_BLOCKING_DOOR,	//This edge has a door that can block it.
		WE_BLOCKING_WALL,
		WE_BLOCKING_BREAK,

		WE_VALID,
		WE_ONHULL,
		WE_FLYING,	//You gotta be able to fly to use this edge.
		WE_JUMPING,
		WE_CANBEINVAL,	//This might be an invalid edge
		WE_DESIGNERPLACED,
	...
	{
		//[test]
		CVec3 Half(GetNode(mNodeA).mPoint);
...
struct	SNodeSort
{
	...
	//racc - used for sorting the nodes by distance value.
...
extern "C"
{
	extern qboolean INV_GoodieKeyCheck( gentity_t *target );
}
	...
	virtual		bool	is_valid(CWayEdge& Edge, int EndPoint=0) const
	{
		...
		if (Edge.mEntityNum!=ENTITYNUM_NONE)
		{
			...
			if (ent)
			{
				...
				if (Edge.BlockingDoor())
				{
					...
					if (Closed)
					{
						...
						if (owner)
						{
							...
							//if ((owner->r.svFlags & SVF_INACTIVE) ||
				...
				{
					return !(ent->r.contents&CONTENTS_SOLID);
				}
			}
		}
...
bool		ViewTrace(const CVec3& a, const CVec3& b)
{
	...
	mViewTraceCount++;
	trap_Trace(&mViewTrace, a.v, 0, 0, b.v, ENTITYNUM_NONE, contents);
					  ...
					  int OverrideContents=0)
{
	...
	//---------------
	mMoveTraceCount++;
	trap_Trace(&mMoveTrace, Start.v, Mins.v, Maxs.v, Stop.v, IgnoreEnt, contents);
...
{
	assert(actor!=0);
	CVec3	Mins(actor->r.mins);
	CVec3	Maxs(actor->r.maxs);

	Mins[2] += (STEPSIZE*1);

	return MoveTrace(actor->r.currentOrigin, goalPosition, Mins, Maxs, actor->s.number, true, true, IgnoreAllEnts/*, actor->contents*/);
}
...
/* NAUM racc - debugging print stuff.  I don't think we need it
...
/* load/save system is disabled by SAVE_LOAD = 0 anyway!
#ifndef _XBOX
	if (SAVE_LOAD)
	{
...
// 
////////////////////////////////////////////////////////////////////////////////////////
bool			NAV::TestEdge( TNodeHandle NodeA, TNodeHandle NodeB, bool IsDebugEdge )
{
	...
	{//racc - either of the nodes has a radius smaller than the edge size.  No fitting thru there.
		if (IsDebugEdge)
		{
			G_Printf("Nav(%s)<->(%s): Size Too Big\n", aName, bName, i);
		}
		CanGo = false;
		return CanGo;
	}
	...
	{//racc - one of the two waypoints is in the air, set the approprate flags on the edge
		at.mFlags.set_bit(CWayEdge::EWayEdgeFlags::WE_FLYING);
		if (!a.mFlags.get_bit(CWayNode::EWayNodeFlags::WN_FLOATING) || !b.mFlags.get_bit(CWayNode::EWayNodeFlags::WN_FLOATING))
		{//one node is in the air and the other isn't, flag that this edge might be invalid
	...
	if (!CanGo && 
		...
		(&g_entities[EntHit])!=0)
	{//racc - this edge is blocked but there appears to be an entity in a way.  Check it out.
		gentity_t*	ent		= &g_entities[EntHit];

		if (IsDebugEdge)
		{
			G_Printf("Nav(%s)<->(%s): Hit Entity Type (%s), TargetName (%s)\n", aName, bName, ent->classname, ent->targetname);
		}
		...
		{//racc - This edge has a door that can block it, flag edge.
		...
		{//racc - some sort of wall object is blocking our path, flag edge.
		...
		{//racc - some sort of breakable object is blocking our path, flag edge.
			at.mFlags.set_bit(CWayEdge::EWayEdgeFlags::WE_BLOCKING_BREAK);
		}
		else if (ent->NPC || ent->s.number < MAX_CLIENTS)
		//else if (ent->NPC || ent->s.number==0) SP Code
		{//racc - some sort of creature is blocking our path.
			HitCharacter = true;
		}
		...
		{//racc - some sort of other type of entity blocking our path.
			if (IsDebugEdge)
			{
				G_Printf("Nav(%s)<->(%s): Unable To Ignore Ent, Going A Size Down\n", aName, bName);
			}
		...
		if (at.BlockingDoor())
		{
			...
			while (slave)
			{
				VectorCopy((DoorIsStartOpen)?(slave->pos1):(slave->pos2), slave->r.currentOrigin);
			...
			CanGo = MoveTrace(a.mPoint, b.mPoint, Mins, Maxs, 0, true, false);
			if (CanGo)
			{//racc - path is clear, set our entity stuff to point to the blocking door's master.
				ent		= master;
			...
			else
			{
				if (IsDebugEdge)
				{
					G_Printf("Nav(%s)<->(%s): Unable Pass Through Door Even When Open, Going A Size Down\n", aName, bName);
				}
			}
			...
			while (slave)
			{
				VectorCopy((DoorIsStartOpen)?(slave->pos2):(slave->pos1), slave->r.currentOrigin);
				trap_LinkEntity(slave);
				slave = slave->teamchain;
			}
		}
		...
		{//RAFIXME - this doesn't look like this is written properly.  It doesn't check CanGo after doing trace.
			CanGo = MoveTrace(a.mPoint, b.mPoint, Mins, Maxs, EntHit, true, false);
			if (IsDebugEdge)
			{
				G_Printf("Nav(%s)<->(%s): Unable Pass Through Even If Entity Was Gone, Going A Size Down\n", aName, bName);
			}
		}
		...
		{//racc - this is a waypoint that can possibly be blocked by something static that we can handle.  
			//Inform the hitEnt and edge about this.
			ent->wayedge		= atHandle;
			at.mEntityNum		= EntHit;
			at.mFlags.set_bit(CWayEdge::EWayEdgeFlags::WE_CANBEINVAL);

			//removed to get past this point for a bit
			// Add It To The Edge Map
			//------------------------
			TEntEdgeMap::iterator eemiter = mEntEdgeMap.find(EntHit);
			if (eemiter==mEntEdgeMap.end())
			{//racc - no TEdgesPerEnt found for this ent in mEntEdgeMap
				TEdgesPerEnt	EdgesPerEnt;
				EdgesPerEnt.push_back(atHandle);
				mEntEdgeMap[EntHit] = EdgesPerEnt;
				//mEntEdgeMap.insert(EntHit, EdgesPerEnt); //SP code version.
			}
			else
			{
				if (!eemiter->second.full())
				{
					eemiter->second.push_back(atHandle);
				}
				else
				{	
#ifndef FINAL_BUILD
					assert(!"Max Edges Perh Handle Reached, Unable To Add To Edge Map!");
					//assert("Max Edges Perh Handle Reached, Unable To Add To Edge Map!"==0); 
					G_Printf("WARNING: Too many nav edges pass through entity %d (%s)\n", EntHit, g_entities[EntHit].targetname);
#endif
				}
			}
...
{//racc - initialize the SP Nav system and saving the results to file.
	int contents[MAX_GENTITIES] = {0};
	...
	mViewTraceCount = 0;
	mConnectTraceCount = 0;
	mConnectTime = trap_Milliseconds();
	...
	if (CHECK_START_OPEN)
	{
		for (int curEnt=0; curEnt<MAX_GENTITIES; curEnt++)
		{
			...
			{//make NPC entities non-solid for this checking.
				NPCs.set_bit(curEnt);
				//RAFIXME - Impliment lastMoveTime?
				contents[ent->s.number] = ent->r.contents;
				//ent->NPC->lastMoveTime = ent->r.contents;
				ent->r.contents = 0;	
				trap_LinkEntity(ent);
			}
			...
			else if (!Q_stricmp("func_wall", ent->classname) ||	!Q_stricmp("func_usable", ent->classname))
			{
				...
				ent->r.contents = ent->spawnContents;	
				trap_LinkEntity(ent);
			}
		}
	}
...
*/
	...
	for (nodeIter=mGraph.nodes_begin(); nodeIter!=mGraph.nodes_end(); nodeIter++)
	{
		...
		for (tgtNum=0; tgtNum<NAV::NUM_TARGETS; tgtNum++)
		{
			...
			if(tgtName.empty())
			//if (!tgtName || tgtName.empty())  //SP Code Version
			{
				continue;
			}
			...
			//-------------------------------------------------------
			at->mTargets[tgtNum].clear();
			//at->mTargets[tgtNum] = 0;  //SP Code Version
			...
			{
#ifndef FINAL_BUILD
				G_Printf(S_COLOR_YELLOW"WARNING: nav unable to locate target (%s) from node (%s)\n", tgtNameStr, atNameStr);
#endif
				continue;
			}
			...
			// For Each One
			//--------------
			for (int tgtNameIndex=0; tgtNameIndex<(*nameFinder).second.size(); tgtNameIndex++)
			{
				// Connect The Two Nodes In The Graph
				//------------------------------------
				tgtHandle	= (*nameFinder).second[tgtNameIndex];
			...
			(at->mType==NAV::PT_COMBATNODE || at->mType==NAV::PT_GOALNODE))
		{
			...
			for (int cellNode=0; cellNode<Cell.mNodes.size(); cellNode++)
			{
				...
				//--------------------------------
				if (!(trap_InPVS(node.mPoint.v, at->mPoint.v)))
				{
					continue;
				}
	...
	for (edgeIter=mGraph.edges_begin(); edgeIter!=mGraph.edges_end(); edgeIter++)
	{
		...
		bool	IsDebugEdge = 
			...
			(!stricmp(a.mName.c_str(), g_nav1.string) || !stricmp(b.mName.c_str(), g_nav1.string)) && 
			(!stricmp(a.mName.c_str(), g_nav2.string) || !stricmp(b.mName.c_str(), g_nav2.string)));
			/* SP Code Version
		...
		{
			G_Printf("===============================\n");
			G_Printf("Nav(%s)<->(%s): DEBUGGING START\n", aName, bName);
			assert(0);	// Break Here
		}
		...
		if (IsDebugEdge)
		{
			G_Printf("Nav(%s)<->(%s): Attempting Size Large...\n", aName, bName);
		}
		...
		if (!CanGo)
		{
			...
			if (IsDebugEdge)
			{
				G_Printf("Nav(%s)<->(%s): Attempting Size Medium...\n", aName, bName);
			}
			CanGo = TestEdge( at.mNodeA, at.mNodeB, IsDebugEdge );
		}
		...
		if (!CanGo)
		{
			...
			if (IsDebugEdge)
			{
				...
				G_Printf("Nav(%s)<->(%s): FAILED, NO SMALLER SIZE POSSIBLE\n", aName, bName);
				G_Printf("Nav(%s)<->(%s): The last trace hit:\n", aName, bName);
				G_Printf("Nav(%s)<->(%s):     at %s,\n", aName, bName, cpointstr);
				G_Printf("Nav(%s)<->(%s):     normal %s\n", aName, bName, cnormstr);
				if (mMoveTrace.entityNum!=ENTITYNUM_WORLD)
				{
					gentity_t*	ent		= &g_entities[mMoveTrace.entityNum];
					G_Printf("Nav(%s)<->(%s):     on entity Type (%s), TargetName (%s)\n", aName, bName, ent->classname, ent->targetname);
				}
				if ((mMoveTrace.contents)&CONTENTS_MONSTERCLIP)
				{
					G_Printf("Nav(%s)<->(%s):     with contents BLOCKNPC\n", aName, bName);
				}
				else if ((mMoveTrace.contents)&CONTENTS_BOTCLIP)
				{
					G_Printf("Nav(%s)<->(%s):     with contents DONOTENTER\n", aName, bName);
				}
				else if ((mMoveTrace.contents)&CONTENTS_SOLID)
				{
					G_Printf("Nav(%s)<->(%s):     with contents SOLID\n", aName, bName);
				}
				else if ((mMoveTrace.contents)&CONTENTS_WATER)
				{
					G_Printf("Nav(%s)<->(%s):     with contents WATER\n", aName, bName);
				}
			}
		}
		else
		{
			if (IsDebugEdge)
			{
				G_Printf("Nav(%s)<->(%s): Success!\n", aName, bName);
			}
		}

		if (IsDebugEdge)
		{
			G_Printf("Nav(%s)<->(%s): DEBUGGING END\n", aName, bName);
			G_Printf("===============================\n");
		}
	}
...
#ifndef FINAL_BUILD
			G_Printf( S_COLOR_RED"ERROR: Nav connect failed: %s@%s <-> %s@%s\n", aHstr.c_str(), mLocStringA, bHstr.c_str(), mLocStringB);
#endif
			//RAFIXME - impliment delayedShutDown?
	...
	for (nodeIter=mGraph.nodes_begin(); nodeIter!=mGraph.nodes_end(); nodeIter++)
	{
		...
		if (!mGraph.node_has_neighbors(atHandle))
		{
			...
			{
#ifndef FINAL_BUILD
				G_Printf( S_COLOR_RED"ERROR: Combat Point %s@%s Is Not Connected To Anything\n", at->mName.c_str(), mLocStringA);
				//RAFIXME - impliment delayedShutDown?
				//delayedShutDown = level.time + 100;
#endif
			}
			if (at->mType==NAV::PT_GOALNODE)
			{
				...
				{
#ifndef FINAL_BUILD
					G_Printf( S_COLOR_RED"ERROR: Nav Goal %s@%s Is Not Connected To Anything\n", at->mName.c_str(), mLocStringA);
					//RAFIXME - impliment delayedShutDown?
					//delayedShutDown = level.time + 100;
#endif
				}
...
//	mAirRegion		= mRegion.reserve();
	...
	{
#ifndef FINAL_BUILD
		G_Printf( S_COLOR_RED"ERROR: Too Many Regions!\n");
		//RAFIXME - impliment delayedShutDown?
		//delayedShutDown = level.time + 100;
#endif
	}
	...
	{
#ifndef FINAL_BUILD
		G_Printf( S_COLOR_RED"ERROR: Too Many Region Edges!\n");
		//RAFIXME - impliment delayedShutDown?
		//delayedShutDown = level.time + 100;
#endif
	}
	...
	if (CHECK_START_OPEN)
	{
		for (int curEnt=0; curEnt<MAX_GENTITIES; curEnt++)
		{
			...
			{
				//RAFIXME - Impliment lastMoveTime?
				ent->r.contents = contents[ent->s.number];
	...
	mConnectTime = trap_Milliseconds() - mConnectTime;
...
void			NAV::RegisterDangerSense(gentity_t* actor, int alertEventIndex)
{
	...
	{
		G_DrawRadius(ae.position, ae.radius, NODE_GOAL);
	}
	//=====================================================
...
{//racc - this entity can no longer block edges (probably because it's been removed).  Only breakables appear to use this.
	if (ent)
	{
		ent->wayedge = 0;
...
{//racc - this function adds the waypoint to the SP Nav code system when the waypoint is spawned.
	if (mGraph.size_nodes()>=NUM_NODES)
	{
#ifndef FINAL_BUILD
		G_Printf( "SpawnedPoint: Max Nav points reached (%d)!\n",NUM_NODES );
#endif
        return;
	}
	...
	CVec3 Start(ent->r.currentOrigin);
	CVec3 Stop(ent->r.currentOrigin);
	Stop[2] += 5.0f;
	...
	if (CHECK_START_SOLID)
	{
		...
		{
			assert(!"ERROR: Nav in solid!");  
			//assert("ERROR: Nav in solid!"==0); //racc - weird SP assert thingy
			G_Printf( S_COLOR_RED"ERROR: Nav(%d) in solid: %s@%s\n", type, pointName, mLocStringA);
			//RAFIXME - impliment delayedShutDown?
			//delayedShutDown = level.time + 100;
			return;
		}
	...
	node.mPoint			= ent->r.currentOrigin;
	...
	if (false && CHECK_START_SOLID)
	{
		...
		if (!MoveTrace(Start, Stop, Mins, Maxs, 0, true, false))
		{
			G_Printf( S_COLOR_YELLOW"WARNING: Nav Moved To Solid, Resetting: (%s)\n", pointName);
			assert(!"WARNING: Nav Moved To Solid, Resetting!");
			//assert("WARNING: Nav Moved To Solid, Resetting!"==0); //racc - weird SP assert thingy
			node.mPoint		= ent->r.currentOrigin;
		}
	}
	...
	if (!node.mName.empty())
	{//racc - this node has name associated with it.
		TNameToNodeMap::iterator		nameFinder	= mNodeNames.find(node.mName);

		if (nameFinder==mNodeNames.end())
		{//racc - this node name isn't on the node name lists yet.
			TNamedNodeList	list;
			list.clear();
			list.push_back(NodeHandle);
			mNodeNames[node.mName] = list;
			//mNodeNames.insert(node.mName, list);  //SP code Version
		}
		else
		{//racc - add node number to the NodeName list assoicated with this node name.
			(*nameFinder).second.push_back(NodeHandle);
		}
	}
}
...
NAV::TNodeHandle		NAV::GetNearestNode(gentity_t* ent, bool forceRecalcNow, NAV::TNodeHandle goal)
{
	...
	if (ent->waypoint==WAYPOINT_NONE || forceRecalcNow || (level.time>ent->noWaypointTime))
	{
		...
		ent->waypoint		= 
			...
			goal, 
			ent->s.number, 
			(ent->client && ent->client->ps.eFlags2&EF2_FLYING));
			//(ent->client && ent->client->moveType==MT_FLYSWIM)); //SP code version
...
NAV::TNodeHandle		NAV::GetNearestNode(const vec3_t& position, NAV::TNodeHandle previous, NAV::TNodeHandle goal, int ignoreEnt, bool allowZOffset)
{
	...
	{//racc - we have edges in our graph.
		// Get The List Of Nodes For This Cell Of The Map
		//------------------------------------------------
		TGraphCells::SCell&			Cell = mCells.get_cell(position[0], position[1]);
		if (Cell.mNodes.empty() && Cell.mEdges.empty())
		{//racc - no nodes or edges in this cell?!
#ifndef FINAL_BUILD
			if (g_developer.value)
			{
				G_Printf("WARNING: Failure To Find A Node Here, Examine Cell Layout\n");
			}
#endif
		...
		//===========================
		{
			...
			for (int i=0; i<Cell.mNodes.size() && !mNearestNavSort.full(); i++)
			{
				...
				{//racc - our previous node/edge isn't in the same region as this waypoint, bias it away.
					NodeSort.mDistance += (100.0f*100.0f);
				}
				if (previous>0 && previous!=NodeSort.mHandle && !mGraph.get_edge_across(previous, NodeSort.mHandle))
				{//racc - our previous waypoint was a node and it doesn't have an edge connecting it to this node.
					NodeSort.mDistance += (200.0f*200.0f);
				}
				if (goal && goal!=NodeSort.mHandle && !NAV::InSameRegion(goal, NodeSort.mHandle))
				{//racc - can't get to our current goal thru this node, bias it way.
			...
			//	}
				...
				//racc - add node to our nearest node list.
			...
			for (int j=0; j<mNearestNavSort.size(); j++)
			{
				...
				{//racc - we're touching this node and it's the closest, choose it.
		...
		//racc - ok, we couldn't find a node that we could use as our closest node, look for edges.
		...
		//==========================
		{
			...
			{//racc - add all legit edges to our sort list.
				CWayEdge&	edge = mGraph.get_edge(Cell.mEdges[i]);

				edge.Point(Point);  //racc - set center point of edge
			...
			for (int j=0; j<mNearestNavSort.size(); j++)
			{
				...
				{//racc - the fastest route to edge line is on edge.
					// Otherwise, We Need To Trace To It
					//-----------------------------------
...
{
	CVec3					actorPos(actor->r.currentOrigin);
...
{//calculate a path for this actor between the actor and target waypoint.
	//if MaxDangerLevel != 1, the path must not exceed that danger level at any time.
	mUser.ClearActor();
	...
	{//there currently isn't a mPathUsers element for this NPC.
		if (mPathUsers.full())
		{
			assert(!"NAV: No more unused path users, possibly change MAX_PATH_USERS");
			//assert("NAV: No more unused path users, possibly change MAX_PATH_USERS"==0); //SP Code Version
			return false;
		}
	...
	{//racc - the start and end points are in different regions.  As such, we can't get there from here.
	...
	if (actor->enemy && actor->enemy->client)
	{
		...
		{
			mUser.SetDangerSpot(actor->enemy->r.currentOrigin, 200.0f);
		}
		else if (
			actor->enemy->client->NPC_class==CLASS_RANCOR ||
			actor->enemy->client->NPC_class==CLASS_WAMPA)
		{
			mUser.SetDangerSpot(actor->enemy->r.currentOrigin, 400.0f);
		}
	}
	...
	if (!puser.mSuccess)
	{//racc - A* wasn't able to find a path
		return puser.mSuccess;
	}

	//racc - valid path found.

	// Grab A Couple "Current Conditions"
	//------------------------------------
	CVec3	At(actor->r.currentOrigin);
	...
	//----------------------------
	float minRadius = Min(actor->r.mins[0], actor->r.mins[1]);
	float maxRadius = Max(actor->r.maxs[0], actor->r.maxs[1]);
	float radius = Max(fabsf(minRadius), maxRadius);

	if (radius>20.0f)
	{//racc - truncate radius.
		radius = 20.0f;
	}
	...
	//---------------------------------------
	{
		...
		{
			if (puser.mPath.full())
			{//racc - path is too long.  actually, I think this will never hit due to the mpath condition for the loop.
				// NAV_TODO: If the path is longer than the max length, we want to store the first valid ones, instead of returning false
				assert(!"This Is A Test To See If We Hit This Condition Anymore...  It Should Be Handled Properly");
				//assert("This Is A Test To See If We Hit This Condition Anymore...  It Should Be Handled Properly"==0); //SP assert version?
			...
			{//racc - looks like air waypoints have a hardcoded reached radius.
		...
		//racc - logically we should have stuff in the user's path now.
	...
	//racc - basically this is to prevent NPCs from initially wandering backwards to get onto closest node.  
	//Instead they will jump onto the nearest path edge and travel from there.
	//-----------------------------
	for (int edge=0; edge<numEdges; edge++)
	{
		...
		{//racc - on line segment
			if (AtDistToEdge<(radius) || (AtDistToEdge<(radius*20.0f) && MoveTrace(At, AtOnEdge, actor->r.mins, actor->r.maxs, actor->s.number, true, true, false)))
			{//touching edge or close and able to walk to it.  cut off the rest of the path and just start from there.
	...
	{//start at logical beginning of path and process until we reach destination.
		...
		{//racc - too much of a Z axis different, just smash and not worry about it.
			AtToNext[2] = 0.0f;
		}
		...
		//racc - waypoints right before a sharp turn should make the NPC slow down.
		//--------------------------------------------------
		if (i!=0 &&			//is there a next point?
			!mGraph.get_node(PPoint.mNode).mFlags.get_bit(CWayNode::EWayNodeFlags::WN_FLOATING)
			)
		{//don't do this at the end of the path or for floating waypoints.
			...
			{//racc - sharp turn ahead, slow down before entering turn.
	...
	{//racc - we have a max danger level acceptable for this path and we went over the limit.  Fail!
		puser.mSuccess = false;
	}

	//racc - logically we should have stuff in the user's path now.  (doublechecking at this point)
	assert(puser.mPath.size()>0);
	return puser.mSuccess;
}
...
//	mUser.ClearDangerSpot();
	...
	for (mSearch.path_begin(); !mSearch.path_end(); mSearch.path_inc())
	{
		...
		{//racc - travelling to this makes us get too close to our danger target.
			puser.mSuccess = false;
			break;
		}
		Prev = Next;
	...
	if (puser.mSuccess)
	{//racc - our path is safe as soon as we get on it, make sure we can safely get to the first node.
		Next = startVec;
...
{//racc - determines if the two given edges or nodes are in the same region.
	if (mRegion.size()>0)
	{
		...
		//racc - if the inputs are edges, convert them to nodes.
		if (actNode<0)
		{
			actNode = mGraph.get_edge(abs(actNode)).mNodeA;
		}
...
bool			NAV::OnNeighboringPoints(gentity_t* actor, gentity_t* target)
{
	...
	{
		if (Distance(actor->r.currentOrigin, target->r.currentOrigin)<NEIGHBORING_DIST)
		{
...
bool			NAV::OnNeighboringPoints(gentity_t* actor, const vec3_t& position)
{
	...
	{
		if (Distance(actor->r.currentOrigin, position)<NEIGHBORING_DIST)
		{
...
{//checks for the current danger level of the path we're currently taking.  This accounts for the current danger alerts
	//on the path's edges and accounts for our current immediate enemy danger if we have one and they are dangerous enough.
	if (!actor)
	{
		assert(!"No Actor!!!");
		//assert("No Actor!!!"==0); //SP Code Version
		return 0.0f;
	}
	int pathUserNum = mPathUserIndex[actor->s.number];
	if (pathUserNum==NULL_PATH_USER_INDEX)
	...
	CVec3		prevPoint(actor->r.currentOrigin);
	...
	if (actor->enemy && actor->enemy->client)
	{
		if (actor->enemy->client->ps.weapon==WP_SABER ||
			...
			actor->enemy->client->NPC_class==CLASS_WAMPA)
		{
			enemyPos = (actor->enemy->r.currentOrigin);
	...
	for (; curPathAt>=0; curPathAt--)
	{
		...
		{// prevNode was an edge or there's an edge between our current path node and our prevNode.
			
			//racc - find the current transistion edge.
			if (prevNode<0)
			{
				curEdge = prevNode;
			}
			...
			//scan the alert list for this edge.  If you find an alert for it that's higher than our current 
			//DangerLevel, replace DangerLevel with this higher value.
		...
		//racc - if we have a dangerous enemy, account it's location/distance in our DangerLevel.
...
unsigned int		NAV::ClassifyEntSize(gentity_t* ent)
{
	if (ent)
	{
		float minRadius = Min(ent->r.mins[0], ent->r.mins[1]);
		float maxRadius = Max(ent->r.maxs[0], ent->r.maxs[1]);
		float radius = Max(fabsf(minRadius), maxRadius);
		float height = ent->r.maxs[2];
...
void			NAV::ShowDebugInfo(const vec3_t& PlayerPosition, int PlayerWaypoint)
{
	...
	if (NAVDEBUG_showNodes || NAVDEBUG_showCombatPoints || NAVDEBUG_showNavGoals)
	{
		for (TGraph::TNodes::iterator atIter=mGraph.nodes_begin(); atIter!=mGraph.nodes_end(); atIter++)
		{
			...
			if (trap_InPVS(PlayerPosition, at.mPoint.v))
			{
				if (at.mType==NAV::PT_WAYNODE && NAVDEBUG_showNodes)
				{
					...
					if (NAVDEBUG_showPointLines)
					{
						...
						{
							G_DrawEdge(at.mPoint.v, atEnd.v, EDGE_NODE_FLOATING );
						}
						else
						{
							G_DrawEdge(at.mPoint.v, atEnd.v, EDGE_NODE_NORMAL );
						}
					}
					else
					*/
					{
						if (at.mFlags.get_bit(CWayNode::EWayNodeFlags::WN_FLOATING))
						{
							G_DrawNode(at.mPoint.v, NODE_FLOATING );
						}
						else
						{
							G_DrawNode(at.mPoint.v, NODE_NORMAL );
						}
					}
					if (NAVDEBUG_showRadius && at.mPoint.Dist2(PlayerPosition)<(at.mRadius*at.mRadius))
					{
						if (at.mFlags.get_bit(CWayNode::EWayNodeFlags::WN_FLOATING))
						{
							G_DrawRadius(at.mPoint.v, at.mRadius, NODE_FLOATING );
						}
						else
						{
							G_DrawRadius(at.mPoint.v, at.mRadius, NODE_NORMAL );
						}
					}
				}
				else if (at.mType==NAV::PT_COMBATNODE && NAVDEBUG_showCombatPoints)
				{
					...
					if (NAVDEBUG_showPointLines)
					{
						G_DrawEdge(at.mPoint.v, atEnd.v, EDGE_NODE_COMBAT );
					}
					else
					*/
					{
						G_DrawCombatPoint(at.mPoint.v, 0);
					}
				}
				else if (at.mType==NAV::PT_GOALNODE && NAVDEBUG_showNavGoals)
				{
					...
					if (NAVDEBUG_showPointLines)
					{
						G_DrawEdge(at.mPoint.v, atEnd.v, EDGE_NODE_GOAL );
					}
					else
					*/
					{
						G_DrawNode(at.mPoint.v, NODE_NAVGOAL);
					}
				}
			}
		}
	...
	if (NAVDEBUG_showEdges)
	{
		for (TGraph::TEdges::iterator atIter=mGraph.edges_begin(); atIter!=mGraph.edges_end(); atIter++)
		{
			...
			if (AvePos.Dist2(PlayerPosition)<(500.0f*500.0f) && trap_InPVS(PlayerPosition, AvePos.v))
			{
				if (mUser.is_valid(at))
				{
					if (at.mFlags.get_bit(CWayEdge::EWayEdgeFlags::WE_JUMPING))
					{
						G_DrawEdge(a.mPoint.v, b.mPoint.v, EDGE_JUMP);
					}
					else if (at.mFlags.get_bit(CWayEdge::EWayEdgeFlags::WE_FLYING))
					{
						G_DrawEdge(a.mPoint.v, b.mPoint.v, EDGE_FLY);
					}
					else if (at.Size()==CWayEdge::EWayEdgeFlags::WE_SIZE_LARGE)
					{
						G_DrawEdge(a.mPoint.v, b.mPoint.v, EDGE_LARGE);
					}
					else
					{
						G_DrawEdge(a.mPoint.v, b.mPoint.v, EDGE_NORMAL);
					}
				}
				else
				{
					G_DrawEdge(a.mPoint.v, b.mPoint.v, EDGE_BLOCKED);
				}
			}
		}
	}
	...
	if (NAVDEBUG_showGrid)
	{
		...
		for (int x=0; x<TGraphCells::SIZEX; x++)
		{
			for (int y=0; y<TGraphCells::SIZEY; y++)
			{
				...
				{
					G_DrawEdge(upleft.v, upright.v, type);
					G_DrawEdge(upright.v, downright.v, type);
				//	G_DrawEdge(downright.v, downleft.v, type);
				//	G_DrawEdge(downleft.v, upleft.v, type);
	...
	*/
	...
	if ( NAVDEBUG_showNearest && player && (player->waypoint!=0 || player->lastWaypoint!=0))
	{
		...
		if (PlayerWaypoint>0)
		{
			...
			G_DrawEdge(PPos.v, node.mPoint.v, (player->waypoint)?(EDGE_NEARESTVALID):(EDGE_NEARESTINVALID));
		}
		else
		{
			...
			G_DrawEdge(PPos.v, PosOnLine.v, (player->waypoint)?(EDGE_NEARESTVALID):(EDGE_NEARESTINVALID));
		}
	}
	*/
}
...
{
	assert(actor!=0);
	hstring	nName(pointName); //SP Code Version
	TNameToNodeMap::iterator	nameFinder= mNodeNames.find(nName);
	if (nameFinder!=mNodeNames.end())
	{
		if ((*nameFinder).second.size()>1)
		{
			G_Printf("WARNING: More than one point named (%s).  Going to first one./n", pointName);
		}
		TeleportPlayer(actor, mGraph.get_node((*nameFinder).second[0]).mPoint.v, actor->r.currentAngles);
		return;
	}
	G_Printf("Unable To Locate Point (%s)\n", pointName);
}
...
{
	assert(actor!=0);
	TeleportPlayer(actor, mGraph.get_node(pointNum).mPoint.v, actor->r.currentAngles);
	return;
}
...
//==================================================
	if (mSteerUsers.full())
	{
		assert(!"STEER: No more unused steer users, possibly change size");
		//assert("STEER: No more unused steer users, possibly change size"==0);  //SP Code Version
		return;
	}
...
//====================================================
	suser.mPosition		= actor->r.currentOrigin;
	suser.mOrientation	= actor->r.currentAngles;
	...
	//RAFIXME - Impliment MT_FLYSWIM?
	CVec3	Range(RangeSize, RangeSize, (actor->client->ps.eFlags2&EF2_FLYING)?(RangeSize):(suser.mRadius*2.0f));
	//CVec3	Range(RangeSize, RangeSize, (actor->client->moveType==MT_FLYSWIM)?(RangeSize):(suser.mRadius*2.0f)); //SP Code
	...
	int	numFound = trap_EntitiesInBox(Mins.v, Maxs.v, EntityList, MAX_GENTITIES);
	for (int i=0; i<numFound; i++) 
	{
		neighbor = &g_entities[EntityList[i]];
		assert(neighbor!=0);
...
//===================================================
	...
	if (suser.mNewtons>1E-10)
	{
		...
		if (NAVDEBUG_showCollision)
		{
			...
			G_DrawEdge(suser.mPosition.v, EndThrust.v,		EDGE_THRUST);
			G_DrawEdge(suser.mPosition.v, EndVelocity.v,	EDGE_VELOCITY);
		}
		//=================================================================
	}
...
//========================================================
	if (!NPC_Jumping())
	{
		...
		if (suser.mSpeed>0.0f && MoveDir!=vec3_origin)
		//if (suser.mSpeed>0.0f && MoveDir!=CVec3::mZero) //SP code version
		{
			MoveDir.Norm();
...
//======================================================
		...
		if (suser.mSpeed > 0.0f)
		{
			...
			AngleVectors(actor->r.currentAngles,	forward, right, up);	// Use Current Angles
			...
			//--------------------------
			//RAFIXME - impliment this?
			if(actor->client->ps.eFlags2&EF2_FLYING)
			//if (actor->client->moveType==MT_FLYSWIM) //SP version
			{
		...
		else
		{
			...
			//--------------------------
			if(actor->client->ps.eFlags2&EF2_FLYING)
			//if (actor->client->moveType==MT_FLYSWIM) //SP Code version
			{
...
//========================================
		...
		if (STEER::HasBeenBlockedFor(actor, 2000))
		{
			...
			{
				G_DrawEdge(actor->r.currentOrigin, actor->NPC->blockedTargetPosition, EDGE_PATHBLOCKED);
				if (actor->waypoint)
				{
					CVec3	Dumb(NAV::GetNodePosition(actor->waypoint));
					G_DrawEdge(actor->r.currentOrigin, Dumb.v, EDGE_BLOCKED);
				}
			}
				...
				)
			{
				...
				if ((STEER::HasBeenBlockedFor(actor, 8000)) && 
					(!actor->waypoint || Distance(NAV::GetNodePosition(actor->waypoint), actor->r.currentOrigin)>150.0f) && 
					(actor->lastWaypoint))
				{
					/* RAFIXME - impliment the this code for MP env
					if (player &&
						(STEER::HasBeenBlockedFor(actor, 15000)) && 
						!trap_InPVS(player->r.currentOrigin, NAV::GetNodePosition(actor->lastWaypoint)) &&
						!trap_InPVS(player->r.currentOrigin, actor->r.currentOrigin) &&
					...
					*/
					{
						NPC_TryJump3(NAV::GetNodePosition(actor->lastWaypoint), 0.0f, 0.0f);
					}
				}
				...
				else
				{
					...
					{
						NPC_TryJump2(NPCInfo->blockedTargetEntity, 0.0f, 0.0f);
					}
					...
					{
						NPC_TryJump3(NPCInfo->blockedTargetPosition, 0.0f, 0.0f);
					}
				}
			}
		}
...
bool			STEER::SafeToGoTo(gentity_t* actor, const vec3_t& targetPosition, int targetNode)
{
	...
	float	actorToTargetDistance	= Distance(actor->r.currentOrigin,  targetPosition);


	// Are They Close Enough To Just Go There
	//----------------------------------------
	if (actorToTargetDistance<110.0f && fabsf(targetPosition[2]-actor->r.currentOrigin[2])<50.0f)
	{
		return true;
	}
	...
	if (actorToTargetDistance<500.0f)
	{
		...
		if (NAV::OnNeighboringPoints(actorNode, targetNode) &&  
			NAV::InSafeRadius(actor->r.currentOrigin, actorNode,  targetNode) && 
			...
			if (TIMER_Done(actor, "SafeToGoToCHECK"))
			{
				...
				if (MoveTrace(actor, targetPosition, true))
				{
					...
					if (NAVDEBUG_showCollision)
					{
						...
						G_DrawEdge(actor->r.currentOrigin, Dumb.v, EDGE_WHITE_TWOSECOND);
					}
				}
				else
				{
					...
					{
						CVec3 Dumb(targetPosition);
						G_DrawEdge(actor->r.currentOrigin, Dumb.v, EDGE_RED_TWOSECOND);
					}
				}
			}
		}
...
bool			STEER::GoTo(gentity_t* actor,  gentity_t* target, float reachedRadius, bool avoidCollisions)
{
	...
	//------------------------------------------------
	if (STEER::Reached(actor, target->r.currentOrigin, reachedRadius, 
		(actor->client && actor->client->ps.eFlags2&EF2_FLYING))) 
	//if (STEER::Reached(actor, target->r.currentOrigin, reachedRadius, (actor->client && actor->client->moveType==MT_FLYSWIM))) //SP code version
	{
		NAV::ClearPath(actor);
		STEER::Stop(actor);
		return true;
	...
	if (
		...
		!STEER::SafeToGoTo(actor, target->r.currentOrigin, NAV::GetNearestNode(target))
 		)
	{
		return false;
	}
	...
	if (NAVDEBUG_showEnemyPath)
	{
		G_DrawEdge(actor->r.currentOrigin, target->r.currentOrigin, EDGE_FOLLOWPOS);
	}
	return true;
}
...
bool			STEER::GoTo(gentity_t* actor, const vec3_t& position, float reachedRadius, bool avoidCollisions)
{
	...
	//------------------------------------------------
	//RAFIXME - impliment MT_FLYSWIM?
	if (STEER::Reached(actor, position, reachedRadius, (actor->client && actor->client->ps.eFlags2&EF2_FLYING)))
	//if (STEER::Reached(actor, position, reachedRadius, (actor->client && actor->client->moveType==MT_FLYSWIM))) //SP code version
	{
		NAV::ClearPath(actor);
		STEER::Stop(actor);
		return true;
	...
	{
		CVec3	Dumb(position);
		G_DrawEdge(actor->r.currentOrigin, Dumb.v, EDGE_FOLLOWPOS);
	}
	return true;
}
...
void			STEER::Blocked(gentity_t* actor, gentity_t* target)
{
	...
	suser.mBlockedTgtPosition	= target->r.currentOrigin;
}
...
float			STEER::Persue(gentity_t* actor,	gentity_t* target, float slowingDistance)
{
	...
	CVec3	ProjectedTargetPosition(target->r.currentOrigin);
...
float			STEER::Persue(gentity_t* actor,	gentity_t* target, float slowingDistance, float offsetForward, float offsetRight, float offsetUp, bool relativeToTargetFacing)
{
	...
	{
		AngleVectors(target->r.currentAngles, ProjectForward.v, ProjectRight.v, ProjectUp.v);
...
float			STEER::Separation(gentity_t* actor, float Scale)
{
	...
	if (!suser.mNeighbors.empty())
	{
		for (int i=0; i<suser.mNeighbors.size(); i++)
		{
			...
			{
				CVec3	NbrPos(suser.mNeighbors[i]->r.currentOrigin);
				...
				if (NbrToActDist>1.0f)
				{
					...
					if (NAVDEBUG_showCollision)
					{
						...
						G_DrawEdge(suser.mPosition.v,	Prj.v, EDGE_IMPACT_POSSIBLE);	// Separation
					}
				}
			}
		}
...
float			STEER::Cohesion(gentity_t* actor, float Scale)
{
	...
	if (!suser.mNeighbors.empty())
	{
		...
		for (int i=0; i<suser.mNeighbors.size(); i++)
		{
			AvePosition += CVec3(suser.mNeighbors[i]->r.currentOrigin);
		}
...
float			STEER::Path(gentity_t* actor)
{
	if (NAV::HasPath(actor))
	{
		...
		{
			assert("STEER: Unable to obtain the next path position"); 
			//assert("STEER: Unable to obtain the next path position"==0);  //SP Code Version
			return 0.0f;
		}
		...
		*/
		...
		if (Jump)
		{
			if (NPC_TryJump3(NextPosition.v, 0.0f, 0.0f))
			{
				//RAFIXME - impliment NPCAI_JUMP?
		...
		if (NAVDEBUG_showEnemyPath)
		{
			CVec3	Prev(actor->r.currentOrigin);
			TPath&	path = mPathUsers[mPathUserIndex[actor->s.number]].mPath;
			for (int i=path.size()-1; i>=0; i--)
			{
				G_DrawEdge(Prev.v, path[i].mPoint.v, EDGE_PATH);
				Prev = path[i].mPoint;
			}
		}
...
float			STEER::Wander(gentity_t* actor)
{
	...
	CVec3 Direction(1, 0, 0);
	//CVec3 Direction(CVec3::mX); //SP code version
...
float			STEER::FollowLeader(gentity_t* actor, gentity_t* leader, float dist)
{
	...
	{
		CVec3	LeaderPosition(leader->r.currentOrigin);
		CVec3	LeaderDirection(leader->r.currentAngles);
	...
	if (NAVDEBUG_showEnemyPath)
	{
		G_DrawEdge(leader->r.currentOrigin, leader->followPos, EDGE_FOLLOWPOS);
	}
...
bool		TestCollision(gentity_t* actor, SSteerUser& suser, const CVec3& ProjectVelocity, float ProjectSpeed, ESide Side, float weight=1.0f)
{
	...
	if (!Safe && Side==Side_None)
	{
		...
		if (ContactNum!=ENTITYNUM_WORLD && ContactNum!=ENTITYNUM_NONE && g_entities[ContactNum].client!=0)
		{
			...
			CVec3			ContactPosition   (Contact->r.currentOrigin);
			...
			else
			{
				...
				CVec3 AbsMin(Contact->r.absmin);
				CVec3 AbsMax(Contact->r.absmax);
		...
		if (!Safe)
		{
			...
			//-------------------------------------------------------------------
			ContactNormal.Cross(0,0,1);
			//ContactNormal.Cross(CVec3::mZ); //SP Code version
	...
	if (NAVDEBUG_showCollision)
	{
		...
		if (Safe)
		{
			G_DrawEdge(suser.mPosition.v,	Prj.v,				EDGE_IMPACT_SAFE);		// WHITE LINE
		}
		else
		{
			G_DrawEdge(suser.mPosition.v,	mMoveTrace.endpos,	EDGE_IMPACT_POSSIBLE);	// RED LINE
			G_DrawEdge(mMoveTrace.endpos,	ContactPoint.v,		EDGE_IMPACT_POSSIBLE);	// RED LINE
		}
	}
	//=================================================================
...
float			STEER::AvoidCollisions(gentity_t* actor, gentity_t* leader)
{
	...
	{
		if (actor->NPC->greetEnt && actor->NPC->greetEnt->r.ownerNum==actor->s.number)
		//if (actor->NPC->greetEnt && actor->NPC->greetEnt->owner==NPC) //SP Code Version
		{
	...
	if (ProjectedSpeed>0.01f)
	{
		...
		ProjectVelocitySide.Reposition(vec3_origin, (actor->NPC->avoidSide==Side_Left)?(40.0f):(-40.0f));
		//ProjectVelocitySide.Reposition(CVec3::mZero, (actor->NPC->avoidSide==Side_Left)?(40.0f):(-40.0f)); //SP Code Version
...
bool	STEER::Reached(gentity_t* actor, gentity_t* target, float targetRadius, bool flying)
{
	...
	CVec3	ActorPos(actor->r.currentOrigin);
	CVec3	ActorMins(actor->r.absmin);
	CVec3	ActorMaxs(actor->r.absmax);

	CVec3	TargetPos(target->r.currentOrigin);
