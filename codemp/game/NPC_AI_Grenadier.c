	...
	*/
	...
	if ( !NPCS.NPCInfo->goalEntity )
	{
		if ( NPCS.NPCInfo->scriptFlags & SCF_CHASE_ENEMIES )
		{
			...
			//[CoOp]
			NPCS.NPCInfo->goalRadius = (NPCS.NPC->r.maxs[0]*1.5f);	//set how close we get to the enemy.
			//[/CoOp]
		}
	}
}
...
void NPC_BSGrenadier_Attack( void )
{
	...
	//[CoOp]
	if ( TIMER_Done( NPCS.NPC, "flee" ) && NPC_CheckForDanger( NPC_CheckAlertEvents( qtrue, qtrue, -1, qfalse, AEL_DANGER, qfalse ) ) )
	//[/CoOp]
	{//going to run
		NPC_UpdateAngles( qtrue, qtrue );
		return;
