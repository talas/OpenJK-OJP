...
//[DynamicMemory_Vehicles]
	...
	//trap->TrueFree(&VehWeaponParms);
	VehWeaponParms[0] = 0;
	trap->TrueFree((void **)&VehicleParms);
#else
...
vehField_t vehicleFields[] =
{
	...
	{"VehicleMovementModesActive", VFOFS(VehicleMovementModesActive), VF_BOOL},	//If FALSE, no vehicle movement
																				//mode stuff is implemented
	//Disable firing for certain modes stuff
	{"MBFstopprimaryfiring", VFOFS(MBFstopprimaryfiring), VF_INT},	   //Stop primary firing in these modes 
																	   //(vehicleMode_t bitfield)
	{"MBFstopaltfiring", VFOFS(MBFstopaltfiring), VF_INT},				//Stop alt firing in these modes
																		 //(vehicleMode_t bitfield)
	{"MBFdisableshields", VFOFS(MBFdisableshields), VF_INT},

	{"CantKnockoutShields", VFOFS(CantKnockoutShields), VF_BOOL},
	{"AllWeaponsDoDamageToArmor", VFOFS(AllWeaponsDoDamageToArmor), VF_BOOL},
	{"AllWeaponsDoDamageToShields", VFOFS(AllWeaponsDoDamageToShields), VF_BOOL},
	{"ResistsMarking", VFOFS(ResistsMarking), VF_BOOL},
	{"SpeedMultiplierForRamDamage", VFOFS(SpeedMultiplierForRamDamage), VF_FLOAT},
	//ROP VEHICLE_IMP END

	//[CloakingVehicles]
	{"CloakingDevice", VFOFS(CloakingDevice), VF_BOOL},	//Does this vehicle have a cloaking
	//device?
	{"CloakingDeviceOnSound", VFOFS(CloakingDeviceOnSound),	VF_SOUND}, //sound played when
	//cloaking device is turned on.
	{"CloakingDeviceOffSound", VFOFS(CloakingDeviceOffSound),	VF_SOUND}, //sound played when
	//cloaking device is turned off.
	//[/CloakingVehicles]
...
*/
	//ROP VEHICLE_IMP START
	//THis is new stuff, so this REALLY needs to get initialised...
	vehicle->VehicleMovementModesActive = 0;

	vehicle->MBFstopprimaryfiring = 0;					
	vehicle->MBFstopaltfiring = 0;
	vehicle->MBFdisableshields = 0;

	vehicle->CantKnockoutShields = 0;
	vehicle->AllWeaponsDoDamageToArmor = 0;
	vehicle->AllWeaponsDoDamageToShields = 0;
	vehicle->ResistsMarking = 0;
	vehicle->SpeedMultiplierForRamDamage = 10.0f;

	//This isn't a parameter I've added, but it wasn't finished in base jka,
	//so I'd better give it a default here
	vehicle->shieldRechargeMS = 1;
	//ROP VEHICLE_IMP END

	//[CloakingVehicles]
	vehicle->CloakingDevice = qfalse;
	//[/CloakingVehicles]
}
...
{
	int			len, totallen, vehExtFNLen = 0, fileCnt, i;
...
//[DynamicMemory_Vehicles]
	...
	trap->TrueMalloc((void **)&VehicleParms, maxLen);
	if(!VehicleParms){
...
#endif //defined(_GAME) || defined(_CGAME)
//[/DynamicMemory_Vehicles]
