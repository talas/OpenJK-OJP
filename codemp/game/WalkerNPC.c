...
static void ProcessMoveCommands( Vehicle_t *pVeh )
{
	...
	{
		//Did we just finish running a trans anim?
		if(pVeh->bTransAnimFlag)
		{
			//We were. Lower the flag
			pVeh->bTransAnimFlag = qfalse;
...
//ROP VEHICLE_IMP START
void SetVehicleMode(Vehicle_t *pVeh, vehicleMode_t eMode)
{
	if(!pVeh->bTransAnimFlag)
	{
		pVeh->eCurrentMode = eMode;
	}
}
//ROP VEHICLE_IMP END
