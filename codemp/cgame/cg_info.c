...
void CG_LoadingString( const char *s ) {
	...
	if(s && *s) {
		Com_Printf(va((char *)psLoading, (char *)s));
		Com_Printf("\n");
	}
...
//	CG_DrawLoadingIcons();
	...
	if ( cg.infoScreenText[0] ) {
		const char *psLoading = CG_GetStringEdString("MENUS", "LOADING_MAPNAME");
		//[OverflowProtection]
		CG_DrawProportionalString( 320, 128-32, va(/*"Loading... %s"*/ (char*) psLoading, cg.infoScreenText), UI_CENTER|UI_INFOFONT|UI_DROPSHADOW, colorWhite );
		//[/OverflowProtection]		
	} else {
    ...
    if (cgs.gametype != GT_SIEGE)
	{
		...
		{
			CG_DrawProportionalString( 320, y, "Force Jump Only", UI_CENTER|UI_INFOFONT|UI_DROPSHADOW, colorWhite );
			y += iPropHeight;
		}
