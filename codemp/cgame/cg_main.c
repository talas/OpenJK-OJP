...
//	CG_LoadingString( "game media" );
	...
	//[Flamethrower]
	cgs.effects.flamethrower = trap->FX_RegisterEffect( "effects/boba/fthrw.efx" );
	//[/Flamethrower]
...
static int CG_PlayCinematic(const char *name, float x, float y, float w, float h) {
	return trap->CIN_PlayCinematic(name, x, y, w, h, 0);
}
...
//	CG_LoadingString( "collision map" );
	...
	{//check to see if the map exists, if it doesn't, drop out with a message that uses the server's mapURL cvar to
		...
		fileHandle_t	f;
		trap->FS_Open(cgs.mapname, &f, FS_READ);
		if(!f)
		{
			trap->Error( ERR_DROP, "Unable to find map %s.\nDownload URL:\n%s\n", cgs.mapname, cgs.mapURL);
		}
		trap->FS_Close(f);
	}
	//[/MapURLs]
