...
static void CG_ParseScores( void ) {
	...
	for ( i=0; i<readScores; i++ ) {
		...
		cg.scores[i].LMSLives			= atoi( CG_Argv( i*SCORE_OFFSET + 13 ) );
		...
		//[ExpSys]
		//added skill points as an item on the scoreboard
		cg.scores[i].skill			= atoi( CG_Argv( i*SCORE_OFFSET + 18 ) );
		//[/ExpSys]
...
static void CG_ParseTeamInfo( void ) {
	...
	if ( transNumber < 0 || transNumber > TEAM_MAXOVERLAY ) {
		trap->Error( ERR_DROP, "CG_ParseTeamInfo: transNumber out of range (%d)", transNumber );
		return;
	}
		...
		if (str[i] == '@' && (i+1) < strLen)
		{
			if (str[i+1] == '@' && (i+2) < strLen)
			{
				...
				{ //@@@ should mean to insert a StringEd reference here, so insert it into buf at the current place
					...
					buf[b] = 0;
					//[CoOp]
        				static char se_text[2][MAX_STRINGED_SV_STRING];      //just incase it's nested
        				static int              se_index = 0;

        				se_index ^= 1;
        				trap->SE_GetStringTextString(stringRef, se_text[se_index], sizeof(se_text[0]));
        				Q_strcat(buf, MAX_STRINGED_SV_STRING, se_text[se_index]);
					//[/CoOp]
					b = strlen(buf);
				}
			}
...
static void CG_BodyQueueCopy(centity_t *cent, int clientNum, int knownWeapon)
{
	...
	if (!cent->ownerRagging)
	{
		...
		//	aNum = (anim->firstFrame+anim->numFrames)-1;
		//}
		trap->G2API_SetAnimIndex(cent->ghoul2, 0, anim->glaIndex);
...
//[CoOp]
static void CG_BriefMenu_f( void ) {
