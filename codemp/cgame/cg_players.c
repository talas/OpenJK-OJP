...
//[ANIMEVENTS]
//This function is no longer nessicary.  BG_ParseAnimationEvtFile automatically does 
		...
		while (k < ci->saber[saberNum].numBlades)
		{
			...
			if (tagBolt == -1)
			{
				...
				{ //guess this is an 0ldsk3wl saber
					...
					if (tagBolt == -1)
					{
						//74145: FIXME this assert was firing in coop tuskens level.
						//assert(0);
					}
					break;
				}
...
void CG_PlayerAnimEventDo( centity_t *cent, animevent_t *animEvent )
{
	...
	case AEV_SOUND:
		{	// are there variations on the sound?
			if ( animEvent->eventData[AED_SOUNDINDEX_START] == 0 && animEvent->stringData != NULL ) // 74145: fix crash with BOTH_KYLE_MISS
			{
				...
				trap->S_StartSound( NULL, cent->currentState.number, channel, CG_CustomSound(cent->currentState.number, va(animEvent->stringData, n)) );
			}
			else
			{
...
void CG_PlayerAmbientEvents ( centity_t *cent )
{
	...
	{
		animEvents = bgAllEvents[cent->eventAnimIndex].torsoAnimEvents;
	...
	{
		animEvents = bgAllEvents[cent->eventAnimIndex].legsAnimEvents;
		EventNum = CheckAnimFrameForEventType ( animEvents, 0, AEV_AMBIENT );
		if( EventNum != -1 )
		{//Find the Ambient event.
			if(  !animEvents[EventNum].eventData[AED_SOUND_PROBABILITY] || animEvents[EventNum].eventData[AED_SOUND_PROBABILITY] > Q_irand(0, 99) )
...
void CG_DoSaber( vec3_t origin, vec3_t dir, float length, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight, int cnum, int bnum )
{
	...
	{	// always add a light because sabers cast a nice glow before they slice you in half!!  or something...
		//[RGBSabers]
		float light = length*1.4f + Q_flrand(0.0f, 1.0f)*3.0f;
...
void CG_DoPrequelSaber( vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight, qboolean doTrail, int cnum, int bnum, int saberType )
{
	...
	effectradius	= ((radius * 1.6 * v1) + Q_flrand(-1.0f, 1.0f) * 0.1f)*radiusmult;
	coreradius		= ((radius * 0.4 * v2) + Q_flrand(-1.0f, 1.0f) * 0.1f)*radiusmult;
	...
	if (!dontDraw)
	{
		...
		for ( i = 0; i < 1; i++ )//was 2 because it would go through architecture and leave saber trails on either side of the brush - but still looks bad if we hit a corner, blade is still 8 longer than hit
		{
			...
			{//player's dodge is getting low, spark their saber hilt a bit as a warning.
					...
					trap->FX_PlayEffectID( cgs.effects.mSparks, org_, efxDir, -1, -1, qfalse );
			}
...
CheckTrail:
	...
	//[/SFXSabers]
		...
		if( cent->currentState.userInt3 & (1 << FLAG_FATIGUED_HEAVY) )
...
int FindGender( const char *modelPath, centity_t *cent )
{
	...
	if(!temp)
	{//bad player model
		trap->Print("Bad modelPath passed to FindGender.\n");
		return GENDER_MALE;
	}
	...
	*temp = '\0';
	...
	fLen = trap->FS_Open(va("%s/sounds.cfg", modelName), &f, FS_READ);
	...
	{
		trap->Print("FindGender Error: Couldnt find sounds.cfg for %s\n", modelName);
	}
...
void CG_HolsteredWeaponRender( centity_t *cent, clientInfo_t *ci, int holsterType ) 
{
	...
	trap->G2API_GetBoltMatrix(cent->ghoul2, 0, offsetBolt, &boltMatrix, cent->turAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
	BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, boltOrg);
		...
		{//need holstered blaster to be rendered
			...
			{//have specialized bolt
				...
				{//don't already have the blaster bolted.
					...
					//now add the blaster
					trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_BLASTER), 0, cent->ghoul2, G2MODEL_BLASTER_HOLSTERED);
		...
		{//need holstered blaster to be rendered
			...
			{//have specialized bolt
				...
				{//don't already have the blaster bolted.
					...
					//now add the blaster
					trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_BLASTER), 0, cent->ghoul2, G2MODEL_BLASTER2_HOLSTERED);
		...
		{//need holstered pistol to be rendered
			...
			{//have specialized bolt
				...
				{//don't already have the pistol bolted.
					...
					//now add the blaster
					trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_BRYAR_PISTOL), 0, cent->ghoul2, G2MODEL_BLASTER2_HOLSTERED);
					trap->G2API_SetBoltInfo(cent->ghoul2, G2MODEL_BLASTER2_HOLSTERED, ci->holster_blaster2);
		...
		{//need holstered pistol to be rendered
			...
			{//have specialized bolt
				...
				{//don't already have the pistol bolted.
					...
					//now add the blaster
					trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_BRYAR_OLD), 0, cent->ghoul2, G2MODEL_BLASTER2_HOLSTERED);
					trap->G2API_SetBoltInfo(cent->ghoul2, G2MODEL_BLASTER2_HOLSTERED, ci->holster_blaster2);
...
SkipTrueView:
	...
	{//player is firing flamethrower, render effect.
		...
		trap->FX_PlayEntityEffectID(cgs.effects.flamethrower, efOrg, axis, -1, -1, -1, -1);	
	}
	//[/Flamethrower]
